				    ...
				    u64 t, int *q, u16 temp)
{
	...
	"Sensor_l = %d sensor = %d header [%04X] div [%d] ts [%lld] %d %d %d\n",
					...
					st->sensor_l[j].div,
					t, q[0], q[1], q[2]);
				inv_push_16bytes_final(st, j, q, t, temp);
			}
		}
	}
	return 0;
...
static int inv_s32_gyro_push(struct inv_mpu_state *st, int i, s32 *calib, u64 t, u16 temp)
{
	if (st->sensor_l[i].on) {
		st->sensor_l[i].counter++;
		if ((st->sensor_l[i].div != 0xffff) &&
			(st->sensor_l[i].counter >= st->sensor_l[i].div)) {
			inv_push_16bytes_final(st, i, calib, t, temp);
...
int inv_push_gyro_data(struct inv_mpu_state *st, s32 *raw, s32 *calib, u64 t, u16 temp)
{
	...
	for (i = 0; i < 2; i++)
		inv_s32_gyro_push(st, gyro_data[i], raw, t, temp);
	for (i = 0; i < 2; i++)
		inv_s32_gyro_push(st, calib_data[i], calib, t, temp);
