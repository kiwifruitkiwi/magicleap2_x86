...
static char dump_reg_bank = BIT_BANK_SEL_0;
static char dump_reg_addr = 0x00;
...
static ssize_t inv_dump_reg_store(struct device *dev,
			struct device_attribute *attr, const char *buf,
			size_t count)
{
	struct iio_dev_attr *this_attr = to_iio_dev_attr(attr);
	...
	switch (this_attr->address) {
	case ATTR_DUMP_REG_ADDR:
		if (data <= 0x7F) {
			dump_reg_addr = data;
		} else {
			pr_err("Register value cannot be greater than 0x7F");
            return -EINVAL;
		}
		break;
	case ATTR_DUMP_REG_BANK:
		if (data <= BIT_BANK_SEL_4) {
			dump_reg_bank = data;
		} else {
			pr_err("Bank value cannot be greater than %d", BIT_BANK_SEL_4);
            return -EINVAL;
		}
		break;
	}
 ...
 * inv_whoami_show()
 */
static ssize_t inv_whoami_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	char data;
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	...
	inv_plat_read(st, REG_WHO_AM_I, 1, &data);
	return snprintf(buf, MAX_WR_SZ, "%d\n", data);
}
			...
			struct device_attribute *attr, char *buf)
{
	...
	// loop over selected bank on "dump_reg_bank" to get all registers
	inv_plat_single_write(st, REG_REG_BANK_SEL, dump_reg_bank);
	bytes_printed += snprintf(buf + bytes_printed,
			MAX_WR_SZ - bytes_printed, "bank %d\n", dump_reg_bank);
	for (ii = 0; ii <= 0x7F; ii++) {
		/* don't read fifo r/w register for bank 0*/
		if (((ii == REG_MEM_R_W) || (ii == REG_FIFO_DATA_REG)) && (dump_reg_bank == BIT_BANK_SEL_0))
			data = 0;
		else
...
/*
 *  inv_dump_reg_show() - Register dump specific reg.
 *  works with:
 *  dump_reg_bank (bank# to use when reading reg)
 *  dump_reg_addr (reg#  to use when reading reg)
 */
static ssize_t inv_dump_reg_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	int data_reg, bank;
	char data;
	...
	data_reg = (int)dump_reg_addr;
	bank     = (int)dump_reg_bank;

	mutex_lock(&indio_dev->mlock);

	inv_plat_single_write(st, REG_REG_BANK_SEL, bank);
	/* don't read fifo r/w register */
	if ((data_reg != REG_MEM_R_W) && (data_reg != REG_FIFO_DATA_REG)) {
		inv_plat_read(st, data_reg, 1, &data);
		bytes_printed = snprintf(buf + bytes_printed, MAX_WR_SZ, "%d\n", data);
	}
...
static DEVICE_ATTR(whoami, S_IRUGO,
			inv_whoami_show, NULL);

/* special run time sysfs entry, read only */
static DEVICE_ATTR(debug_reg_dump, S_IRUGO | S_IWUSR,
			inv_reg_dump_show, NULL);

static DEVICE_ATTR(dump_reg_data, S_IRUGO | S_IWUSR,
			inv_dump_reg_show, NULL);
...
static IIO_DEVICE_ATTR(in_dump_reg_addr, S_IRUGO | S_IWUSR,
			inv_attr_show, inv_dump_reg_store,
			ATTR_DUMP_REG_ADDR);
static IIO_DEVICE_ATTR(in_dump_reg_bank, S_IRUGO | S_IWUSR,
			inv_attr_show, inv_dump_reg_store,
			ATTR_DUMP_REG_BANK);
...
static const struct attribute *inv_raw_attributes[] = {
	...
	&iio_dev_attr_in_dump_reg_addr.dev_attr.attr,
	&iio_dev_attr_in_dump_reg_bank.dev_attr.attr,
};
