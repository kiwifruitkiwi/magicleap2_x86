...
int inv_mpu_initialize(struct inv_mpu_state *st)
{
	...
	/* Notch and AAF filter for GYRO settings */
	result |= inv_plat_single_write(st, REG_REG_BANK_SEL, BIT_BANK_SEL_1);
	result |= inv_plat_single_write(st, REG_GYRO_CONFIG_STATIC2, BIT_GYRO_NF_DIS);
	result |= inv_plat_single_write(st, REG_GYRO_CONFIG_STATIC3, BIT_GYRO_AAF_DELT_303HZ);
	result |= inv_plat_single_write(st, REG_GYRO_CONFIG_STATIC4, BIT_GYRO_AAF_DELTSQR_303HZ);
	result |= inv_plat_single_write(st, REG_GYRO_CONFIG_STATIC5, BIT_GYRO_AAF_BITSHIFT_303HZ);
	result |= inv_plat_single_write(st, REG_REG_BANK_SEL, BIT_BANK_SEL_0);

	/* AAF filter for ACCEL settings */
	result |= inv_plat_single_write(st, REG_REG_BANK_SEL, BIT_BANK_SEL_2);
	result |= inv_plat_single_write(st, REG_ACCEL_CONFIG_STATIC2, BIT_ACCEL_AAF_DELT_303HZ);
	result |= inv_plat_single_write(st, REG_ACCEL_CONFIG_STATIC3, BIT_ACCEL_AAF_DELTSQR_303HZ);
	result |= inv_plat_single_write(st, REG_ACCEL_CONFIG_STATIC4, BIT_ACCEL_AAF_BITSHIFT_303HZ);
	result |= inv_plat_single_write(st, REG_REG_BANK_SEL, BIT_BANK_SEL_0);
