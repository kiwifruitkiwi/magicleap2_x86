 ...
 *
 * Author: AMD PSP Linux Driver Team
 *
...
enum tee_cmd_id {
	...
	TEE_CMD_ID_GET_DEBUG_TOKEN,
	TEE_CMD_ID_START_TA_DEBUG,
	TEE_CMD_ID_STOP_TA_DEBUG,
};
...
/**
 * get_shift_dirpath() - Gets shift register value.
 *
 * This function can be used by CVIP AMD-TEE driver to update system memory
 * address with updated shift register value because CVIP FMR C-interface is
 * divided into 16GB region
 *
 * Returns:
 * register value if the sp master device is set
 * 0              if there is no sp master device available
 */
...
static inline u64 sp_get_shift_dirpath(void)
{
	return 0;
}
...
#ifdef CONFIG_ARCH_MERO_CVIP
#define SYS_DRAM_APERTURE_BASE 0x800000000
#define ca2sa(x) (((x) | sp_get_shift_dirpath()) - SYS_DRAM_APERTURE_BASE)
#else
#define ca2sa(x) (x)
#endif
