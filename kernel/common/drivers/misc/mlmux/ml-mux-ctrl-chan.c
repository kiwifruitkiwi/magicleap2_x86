	...
	struct ml_mux_chan *chan, bool open)
{
	...
	if (ml_mux_send_msg(mux->client.ch, len, &ctrl_msg))
		...
		dev_info(mux->ctrl->dev, "%s: remote state: %s for %s",
			__func__, open ? "open" : "close", chan->name);
}

static void ml_mux_ctrl_send_open_chans(struct work_struct *work)
{
	struct ml_mux *mux = container_of(work, struct ml_mux, open_ch_work);
	...
	dev_info(mux->ctrl->dev, "%s: remote_is_up set to true", __func__);
				...
				struct ml_mux_hello_msg *h_msg, bool is_ack)
{
	...
	queue_work(system_highpri_wq, &mux->open_ch_work);
}
				...
				struct ml_mux_open_close_msg *msg)
{
	...
	if (!chan) {
		...
		dev_info(dev, "%s: registered channel %s (%d) from remote",
		...
		chan->remote_num = msg->chan;
		dev_info(dev, "%s: remote channel %s (%d) connected",
			__func__, chan->name, chan->remote_num);
				...
				uint32_t len, void *buf)
{
	...
	case ML_MUX_CTRL_MSG_RESET:
		dev_warn(mux->ctrl->dev, "%s: ignore reset msg\n", __func__);
		break;

	default:
		dev_err(mux->ctrl->dev, "unknown msg %d\n", msg->id);
		break;
	}
}
...
int ml_mux_init_ctrl_client(struct ml_mux *mux)
{
	...
	INIT_WORK(&mux->open_ch_work, ml_mux_ctrl_send_open_chans);
	...
	if (ret)
		...
		dev_info(dev, "%s: hello sent to remote\n", __func__);
