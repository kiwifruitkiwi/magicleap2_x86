...
int ml_mux_send_msg(struct ml_mux_chan *chan, uint32_t len, void *msg)
{
	...
	mux = chan->mux;
	if (!mux) {
		dev_err(dev, "mux is null\n");
		return -EINVAL;
	}

	if (!mux->remote_is_up && chan->remote_num != CTRL_CHAN) {
		dev_err(dev, "%s: mux remote is not up\n", __func__);
		return -ENOTCONN;
	}

	if (chan->remote_num == INVALID_CHAN) {
		dev_err(dev, "%s: remote channel is not open\n", __func__);
		return -ENOTCONN;
	}

	if ((len + HEADER_SIZE) > mux->ctrl->max_tx_len) {
		dev_err(dev, "%s: write length outside of ctrl limit range\n",
			__func__);
		return -EINVAL;
	}
...
void ml_mux_msg_receive(struct ml_mux_ctrl *ctrl, uint32_t len, void *msg)
{
	...
	if ((chan->local_num != CTRL_CHAN && !mux->remote_is_up) ||
	    chan->remote_num == INVALID_CHAN) {
		dev_err(ctrl->dev, "%s: msg without handshake ch=%s rn=%d\n",
			__func__, chan->name, chan->remote_num);
		goto unlock;
	}
...
#ifdef CONFIG_OF
static struct ml_mux *ml_mux_process_of(struct ml_mux_client *client)
{
...
#else
static inline struct ml_mux *ml_mux_process_of(struct ml_mux_client *client)
{
...
#ifdef CONFIG_ACPI
static struct ml_mux *ml_mux_process_acpi(struct ml_mux_client *client)
{
	...
	fwnode = acpi_fwnode_handle(ACPI_COMPANION(client->dev));
	if (!fwnode) {
		dev_info(client->dev, "client fwnode not found\n");
		return ERR_PTR(-ENODEV);
	}

	memset(&args, 0x00, sizeof(args));
	ret = __acpi_node_get_property_reference(fwnode, "mlmux-ctrl",
						 0, 0, &args);
	if (ret) {
		dev_err(client->dev, "mlmux-ctrl param not found\n");
		mux = ERR_PTR(-EINVAL);
		goto exit;
	}
	...
	mutex_lock(&ctrl_mutex);
	list_for_each_entry(mux_idx, &ctrl_list, list_entry) {
		adev = ACPI_COMPANION(mux_idx->ctrl->dev);
		fwnode = acpi_fwnode_handle(adev);
		if (fwnode == args.fwnode) {
			mux = mux_idx;
			break;
		}
	}
...
#else
static inline struct ml_mux *ml_mux_process_acpi(struct ml_mux_client *client)
{
	return ERR_PTR(-ENODEV);
}
#endif /* CONFIG_ACPI */

static inline struct ml_mux *ml_mux_process_fwnode(struct ml_mux_client *client)
{
	if (client->dev->of_node)
		return ml_mux_process_of(client);
	else
		return ml_mux_process_acpi(client);
}
...
int ml_mux_request_channel(struct ml_mux_client *client, char *name)
{
	...
	mux = ml_mux_process_fwnode(client);
	if (IS_ERR(mux)) {
		ret = PTR_ERR(mux);
		goto exit;
	} else if (!mux) {
		dev_info(client->dev, "mlmux-ctrl not yet registered\n");
		ret = -EPROBE_DEFER;
		goto exit;
	}
	...
	if (!chan) {
		...
		dev_info(client->dev, "%s: registered %s channel", __func__,
			chan->name);
	} else if (!chan->cl) {
		if (!try_module_get(mux->ctrl->dev->driver->owner)) {
			ret = -ENODEV;
...
static int ml_mux_ctrl_sanity_check(struct ml_mux_ctrl *ctrl)
{
	...
	if (!ctrl->dev->of_node && !ACPI_HANDLE(ctrl->dev)) {
		dev_err(ctrl->dev, "reg ml_mux ctrl '%s' with no dev fwnode!\n",
			ctrl->name);
		ret = -EINVAL;
		goto exit;
	}
...
int ml_mux_ctrl_register(struct ml_mux_ctrl *ctrl)
{
	...
	if (ret) {
		dev_err(ctrl->dev, "failed to create control client\n");
		goto exit_list_del;
	}
...
void ml_mux_reset_channels(struct ml_mux *mux)
{
	...
	if (!idr_is_empty(&mux->chan_idr)) {
		idr_for_each_entry(&mux->chan_idr, chan, i) {
			...
			dev_info(mux->ctrl->dev, "%s: channel reset %s\n",
				__func__, chan->name);
			chan->cl->notify_open(chan->cl, false);
			chan->remote_num = INVALID_CHAN;
		}
