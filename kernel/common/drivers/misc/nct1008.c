				...
				struct thermal_cooling_device *cdev)
{
	...
	for (i = 0; i < sensor_data->num_trips; i++) {
		if (!sensor_data->trips[i].cdev_type)
			continue;
...
static int nct1008_get_crit_temp(int sensor, struct thermal_zone_device *thz,
						int *temp)
{
	struct nct1008_data *data = thz->devdata;

	*temp = data->plat_data.sensors[sensor].shutdown_limit * 1000;
	return 0;
}

static inline int nct1008_loc_get_crit_temp(struct thermal_zone_device *thz,
						int *temp)
{
	return nct1008_get_crit_temp(LOC, thz, temp);
}

static inline int nct1008_ext_get_crit_temp(struct thermal_zone_device *thz,
						int *temp)
{
	return nct1008_get_crit_temp(EXT, thz, temp);
}
...
static struct thermal_zone_device_ops nct_loc_ops = {
	...
	.get_crit_temp = nct1008_loc_get_crit_temp,
...
static struct thermal_zone_device_ops nct_ext_ops = {
	...
	.get_crit_temp = nct1008_ext_get_crit_temp,
...
#ifdef CONFIG_ACPI
static int nct1008_get_settings_pkg(struct i2c_client *client,
				   struct nct1008_platform_data *pdata)
{
	acpi_status status;
	...
	int i = 0;
	static char loc_name[THERMAL_NAME_LENGTH];

	status = acpi_evaluate_object(ACPI_HANDLE(&client->dev),
				"PKGS", NULL, &output);
	if (ACPI_FAILURE(status)) {
		dev_err(&client->dev, "Cannot find PKGS method\n");
		goto err;
	}
	...
	if (strncmp(elem1->string.pointer, "sensor-name", 11) == 0) {
		strlcpy(loc_name, elem2->string.pointer, sizeof(loc_name));
		pdata->loc_name = loc_name;
	} else {
		goto err_free_ptr;
	}
	...
	elem1 = &p->package.elements[++i];
	elem2 = &p->package.elements[++i];
	if (strncmp(elem1->string.pointer, "conv-rate", 9) == 0)
		pdata->conv_rate = elem2->integer.value;
	else
		goto err_free_ptr;

	elem1 = &p->package.elements[++i];
	elem2 = &p->package.elements[++i];
	if (strncmp(elem1->string.pointer, "supported-hwrev", 15) == 0)
		pdata->supported_hwrev = elem2->integer.value;
	else
		goto err_free_ptr;

	elem1 = &p->package.elements[++i];
	elem2 = &p->package.elements[++i];
	if (strncmp(elem1->string.pointer, "extended-range", 14) == 0)
		pdata->extended_range = (bool) elem2->integer.value;
	else
		goto err_free_ptr;

	elem1 = &p->package.elements[++i];
	elem2 = &p->package.elements[++i];
	if (strncmp(elem1->string.pointer, "offset", 6) == 0)
		pdata->offset = (int) elem2->integer.value;
	else
		goto err_free_ptr;

	elem1 = &p->package.elements[++i];
	elem2 = &p->package.elements[++i];
	if (strncmp(elem1->string.pointer, "loc_shutdown-limit", 18) == 0)
		pdata->sensors[LOC].shutdown_limit =
			(int) elem2->integer.value;
	else
		goto err_free_ptr;

	elem1 = &p->package.elements[++i];
	elem2 = &p->package.elements[++i];
	if (strncmp(elem1->string.pointer, "ext_shutdown-limit", 18) == 0)
		pdata->sensors[EXT].shutdown_limit =
			(int) elem2->integer.value;
	else
		goto err_free_ptr;
	...
	elem1 = &p->package.elements[++i];
	elem2 = &p->package.elements[++i];
	if (strncmp(elem1->string.pointer, "loc_suspend_limit_hi", 20) == 0)
		pdata->sensors[LOC].suspend_limit_hi =
			(int) elem2->integer.value;
	else
		pdata->sensors[LOC].suspend_limit_hi = 0;

	elem1 = &p->package.elements[++i];
	elem2 = &p->package.elements[++i];
	if (strncmp(elem1->string.pointer, "loc_suspend_limit_lo", 20) == 0)
		pdata->sensors[LOC].suspend_limit_lo =
			(int) elem2->integer.value;
	else
		pdata->sensors[LOC].suspend_limit_lo = 0;

	elem1 = &p->package.elements[++i];
	elem2 = &p->package.elements[++i];
	if (strncmp(elem1->string.pointer, "ext_suspend_limit_hi", 20) == 0)
		pdata->sensors[EXT].suspend_limit_hi =
			(int) elem2->integer.value;
	else
		pdata->sensors[EXT].suspend_limit_hi = 0;

	elem1 = &p->package.elements[++i];
	elem2 = &p->package.elements[++i];
	if (strncmp(elem1->string.pointer, "ext_suspend_limit_lo", 20) == 0)
		pdata->sensors[EXT].suspend_limit_lo =
			(int) elem2->integer.value;
	else
		pdata->sensors[EXT].suspend_limit_lo = 0;
...
static struct nct1008_platform_data *nct1008_acpi_parse(struct i2c_client
				*client)
{
	struct nct1008_platform_data *pdata;
	...
	if (nct1008_get_settings_pkg(client, pdata))
		goto err_parse_acpi;
...
err_parse_acpi:
	dev_err(&client->dev, "Parsing ACPI data error.\n");
	return NULL;
}
#else
static struct nct1008_platform_data *nct1008_acpi_parse(struct i2c_client
				...
				const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		pdata = nct1008_acpi_parse(client);
	}
	if (!pdata)
		return -ENODEV;
	...
	if (client->dev.of_node) {
		...
		/* Config for the Local sensor. */
		if (sensor_data->shutdown_limit) {
			sensor_data->trips[sensor_data->num_trips].trip_temp =
				sensor_data->shutdown_limit * 1000;
			sensor_data->trips[sensor_data->num_trips].trip_type =
...
#ifdef CONFIG_ACPI
static const struct acpi_device_id nct1008_acpi_match[] = {
	{ "TMP0451", TMP451 },
	{ },
};
MODULE_DEVICE_TABLE(acpi, lm90_acpi_match);
#endif
...
static struct i2c_driver nct1008_driver = {
	.driver = {
		...
		.acpi_match_table = ACPI_PTR(nct1008_acpi_match),
	},
	.probe		= nct1008_probe,
