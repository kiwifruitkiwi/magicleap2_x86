...
extern uint64_t gac_time_get(void);
...
static irqreturn_t ml_vicon_raven_isr(int irq, void *data)
{
	struct ml_vicon_raven_data *vdata = (struct ml_vicon_raven_data *) data;

	vdata->ts = gac_time_get();
	return IRQ_WAKE_THREAD;
}

static irqreturn_t ml_vicon_raven_threaded_isr(int irq, void *data)
{
	struct ml_vicon_raven_data *vdata = (struct ml_vicon_raven_data *) data;
...
static int setup_vsync_irq(struct ml_vicon_raven_data *vdata, u8 edges) {
	int rc;
	int irq_flags = 0;
	...
	rc =  devm_request_threaded_irq(vdata->dev, vdata->vsync_irq,
					ml_vicon_raven_isr, ml_vicon_raven_threaded_isr,
					irq_flags | IRQF_ONESHOT,
					"ml_vicon_raven_vsync", vdata);
	if (rc) {
		dev_err(vdata->dev, "Failed to enable vicon raven irq, %d\n", rc);
		return rc;
	}
			       ...
			       char *buf)
{
	...
	struct ml_vicon_raven_data *vdata = dev_get_platdata(dev);
...
#ifdef CONFIG_PM
static int ml_vicon_raven_suspend(struct device *dev)
{
	struct ml_vicon_raven_data *vdata = dev_get_drvdata(dev);
...
static int ml_vicon_raven_resume(struct device *dev)
{
	struct ml_vicon_raven_data *vdata = dev_get_drvdata(dev);
...
static const struct dev_pm_ops ml_vicon_raven_ops = {
	.suspend = ml_vicon_raven_suspend,
	.resume = ml_vicon_raven_resume,
};
#endif

static int ml_vicon_raven_probe(struct platform_device *pdev)
{
	struct ml_vicon_raven_data *vdata;
...
static int ml_vicon_raven_remove(struct platform_device *pdev)
{
	struct ml_vicon_raven_data *vdata = dev_get_drvdata(&pdev->dev);
...
#ifdef CONFIG_ACPI
static const struct acpi_device_id ml_vicon_raven_acpi_match[] = {
	{"MDA0000", 0},
	{"", 0},
};
MODULE_DEVICE_TABLE(acpi, ml_vicon_raven_acpi_match);
#endif

static struct platform_driver ml_vicon_raven_driver = {
	.driver = {
		.name = "ml_vicon_raven",
		.owner = THIS_MODULE,
#ifdef CONFIG_PM
		.pm = &ml_vicon_raven_ops,
#endif
		.acpi_match_table = ACPI_PTR(ml_vicon_raven_acpi_match),
	},
	.probe = ml_vicon_raven_probe,
	.remove = ml_vicon_raven_remove,
};
module_platform_driver(ml_vicon_raven_driver);

MODULE_ALIAS("Magic Leap, Inc.");
MODULE_DESCRIPTION("Motion capture for Vicon on raven");
MODULE_LICENSE("GPL");
