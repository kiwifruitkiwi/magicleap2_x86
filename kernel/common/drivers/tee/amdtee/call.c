				    ...
				    struct tee_operation *amd)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (type > TEE_OP_PARAM_TYPE_VALUE_INOUT) {
			...
			__dma_map_area((const void *)tee[i].u.memref.shm->kaddr, tee[i].u.memref.size, DMA_TO_DEVICE);
#endif
...
#ifdef CONFIG_ARCH_MERO_CVIP
			__dma_map_area((const void *)tee[i].u.memref.shm->kaddr,
				       tee[i].u.memref.size, DMA_FROM_DEVICE);
#endif
...
int handle_map_shmem(u32 count, struct shmem_desc *start, u32 *buf_id)
{
	...
	/* Create buffer list */
	for (i = 0; i < count ; i++) {
		paddr = ca2sa(__psp_pa(start[i].kaddr));
...
int handle_get_debug_token(struct tee_ioctl_get_debug_token_data *arg)
{
	...
	if (ret) {
		arg->ret = TEEC_ERROR_COMMUNICATION;
		pr_err("%s:submit to ring failed. 0x%x\n", __func__, ret);
		return ret;
	}

	if (arg->ret == TEEC_SUCCESS) {
		if (copy_to_user((void *) arg->debug_token, &cmd.token[0],
					sizeof(cmd.token)))
			return -EFAULT;
		arg->debug_token_size = sizeof(cmd.token);
	} else
		arg->debug_token_size = 0;

	return ret;
}

int handle_start_ta_debug(struct tee_ioctl_start_ta_debug_data *arg,
		struct dlm_manager *dlm_manager)
{
	struct tee_cmd_start_ta_dbg cmd = { {0} };
	phys_addr_t dlm_buf_paddr;
	int ret = 0;

	if ((arg == NULL) || (dlm_manager == NULL))
		return -EINVAL;

	memcpy(cmd.ta_uuid, arg->ta_uuid, sizeof(cmd.ta_uuid));

	dlm_buf_paddr = ca2sa(dlm_manager->buffer_dma);
	...
	cmd.size = dlm_manager->buffer_size;
	...
	if (ret) {
		...
		pr_info("%s:CMD status 0x%x\n", __func__, ret);
		if (arg->ret == TEEC_SUCCESS) {
			arg->dlm_session_id = cmd.sid;
			dlm_manager->dlm_session_id = cmd.sid;
		}
	}
...
int handle_stop_ta_debug(struct tee_ioctl_stop_ta_debug_data *arg)
{
	int ret = 0;
