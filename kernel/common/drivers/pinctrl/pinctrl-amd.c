...
static int amd_gpio_probe(struct platform_device *pdev)
{
	...
	if (!alloc_cpumask_var(&cpumask, GFP_KERNEL)) {
		ret = -ENOMEM;
		dev_err(&pdev->dev, "Failed to init cpumask\n");
		goto out2;
	}

	/* IRQ affinity-mask is an optional resource */
	if (!device_property_read_string(&pdev->dev,
					 "affinity-mask",
					  &irq_affinity) &&
	    !cpumask_parse(irq_affinity, cpumask)) {
		ret = irq_set_affinity_hint(irq_base, cpumask);
		if (ret)
			dev_err(&pdev->dev, "Failed to set affinity\n");
	}
