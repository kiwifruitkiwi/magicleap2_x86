...
int i2c_add_adapter(struct i2c_adapter *adapter)
{
	...
	if (ACPI_COMPANION(dev)) {
		if (!device_property_read_u32(dev, "i2c-bus-number", &id)) {
			adapter->nr = id;
			return __i2c_add_numbered_adapter(adapter);
		}
	}
	dynbus = i2c_acpi_get_first_dyn_bus();

	mutex_lock(&core_lock);
	if (__i2c_first_dynamic_bus_num > dynbus)
		dynbus = __i2c_first_dynamic_bus_num;
	id = idr_alloc(&i2c_adapter_idr, adapter, dynbus, 0, GFP_KERNEL);
	mutex_unlock(&core_lock);
	if (WARN(id < 0, "couldn't get idr"))
		return id;
...
static int __init i2c_init(void)
{
	...
	if (retval < 0)
		retval = i2c_acpi_get_first_dyn_bus();
