...
void i2c_acpi_register_devices(struct i2c_adapter *adap)
{
	...
	status = acpi_walk_namespace(ACPI_TYPE_DEVICE, ACPI_HANDLE(&adap->dev),
...
static acpi_status
i2c_acpi_lookup_bus(acpi_handle handle, u32 lvl, void *data, void **ret)
{
	struct acpi_device *adev;
	int *highest_bus = data;
	int id;
	...
	if (fwnode_property_read_u32(acpi_fwnode_handle(adev), "i2c-bus-number", &id))
		return AE_OK;
...
int i2c_acpi_get_first_dyn_bus(void)
{
	acpi_status status;
	int busnum = -ENOSYS;

	/* Find the highest static bus */
	status = acpi_walk_namespace(ACPI_TYPE_DEVICE, ACPI_ROOT_OBJECT,
				     I2C_ACPI_MAX_SCAN_DEPTH,
				     i2c_acpi_lookup_bus, NULL,
				     &busnum, NULL);

	return ACPI_FAILURE(status) ? -ENOSYS : busnum + 1;
}
