...
static void accumulate_core_delta(struct amd_energy_data *data)
{
	...
	cpu = data->core_cpu_map[data->core_id];
	...
	c_accum = &data->accums[data->core_id];
			   ...
			   u32 attr, int channel, long *val)
{
	...
	if (channel >= data->nr_cpus) {
		int node = channel - data->nr_cpus;
		cpu = cpumask_first_and(cpu_online_mask,
					cpumask_of_node(node));
		amd_add_delta(data, channel, cpu, val, false);
	} else {
		cpu = data->core_cpu_map[channel];
		if (!cpu_online(cpu))
			return -ENODEV;
			     ...
			     u8 type, u32 config)
{
	...
	/* config should be a NULL-terminated list */
	...
	core_cpu_map = devm_kcalloc(dev, cpus,
				    sizeof(int), GFP_KERNEL);
	if (!core_cpu_map)
		return -ENOMEM;
	memset(core_cpu_map, -1, sizeof(int) * cpus);
	for_each_possible_cpu(cpu) {
		core = topology_core_id(cpu);
		if (core < cpus && core_cpu_map[core] == -1)
			core_cpu_map[core] = cpu;
	}
	data->core_cpu_map = core_cpu_map;
...
static const struct x86_cpu_id cpu_ids[] __initconst = {
	{ X86_VENDOR_AMD, 0x17, X86_MODEL_ANY, X86_FEATURE_ANY },
	{}
};
