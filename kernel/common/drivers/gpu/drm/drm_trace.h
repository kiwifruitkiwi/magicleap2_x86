...
TRACE_EVENT(drm_vblank_event,
	    TP_PROTO(int crtc, unsigned int seq, long long int now),
	    TP_ARGS(crtc, seq, now),
	    TP_STRUCT__entry(
		    __field(int, crtc)
		    __field(unsigned int, seq)
		    __field(long long int, now)
		    ),
	    TP_fast_assign(
		    __entry->crtc = crtc;
		    __entry->seq = seq;
	    ...
	    TP_printk("crtc=%d, seq=%u now=%lld", __entry->crtc, __entry->seq, \
			 __entry->now)
);
