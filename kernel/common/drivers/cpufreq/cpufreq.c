...
static int cpufreq_init_policy(struct cpufreq_policy *policy)
{
	...
	if (has_target()) {
		if (speed) {
			/* If speed is requested, set governor to userspace */
			ret = cpufreq_frequency_table_get_index(policy, speed);
			if (ret != -EINVAL)
				gov = find_governor("userspace");
		}
		if (speed && !gov)
			pr_warn("CPU%u: Requested frequency %u not valid\n",
				policy->cpu, speed);

		if (!gov) {
			/*
			 * Update policy governor to the one used before
			 * hotplug.
			 */
...
static int cpufreq_online(unsigned int cpu)
{
	...
	if (speed && !strcmp(policy->governor->name, "userspace") &&
	    cpufreq_frequency_table_get_index(policy, speed) != -EINVAL) {
		snprintf(str_speed, sizeof(str_speed), "%u\n", speed);
		store_scaling_setspeed(policy, str_speed, sizeof(str_speed));
	}
