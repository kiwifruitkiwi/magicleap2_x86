...
const u16 crc_16_l_table[CRC_TAB_SIZE] = {
...
static void diag_handle_hdlc_encode(struct diag_send_desc_type *src_desc,
				    struct diag_hdlc_dest_type *enc)
{
	...
	/* This condition needs to include the possibility of
	 * 2 dest bytes for an escaped byte
	 */
...
void diag_hdlc_encode(struct diag_send_desc_type *src_desc,
		      struct diag_hdlc_dest_type *enc)
{
	if (src_desc && enc && enc->dest && enc->dest_last)
		diag_handle_hdlc_encode(src_desc, enc);
}

int diag_hdlc_decode(struct diag_hdlc_decode_type *hdlc)
{
	u8 *src_ptr = NULL, *dest_ptr = NULL;
...
static uint16_t crc_16_l_calc(u8 *buf_ptr, uint32_t len)
{
	u16 data, crc_16;
...
{
	u16 crc = CRC_16_L_SEED;

	/**
	 * The minimum length of a valid incoming packet is 4
	 * 1 byte of data and 3 bytes for CRC
	 */
	if (!buf || len < 4) {
	...
	crc = crc_16_l_calc(buf, (len - 1) * 8);
	if (crc != CRC_16_L_OK) {
		pr_err("%s: CRC mismatched!\n", __func__);
		return -1;
	}
