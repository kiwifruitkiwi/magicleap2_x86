...
error_ready:
	mhi_cntrl->status_cb(mhi_cntrl, MHI_CB_BOOTUP_TIMEOUT);
...
void mhi_pm_m1_transition(struct mhi_controller *mhi_cntrl)
{
	...
	if (atomic_read(&mhi_cntrl->dev_wake))
		dev_dbg(dev, "M2 transition request post dev_wake:%d\n",
...
int mhi_sync_power_up(struct mhi_controller *mhi_cntrl)
{
	...
	dev_err(dev, "MHI did not reach mission mode within %d ms\n",
			       ...
			       bool in_panic)
{
	...
	if (!timeout_us) {
		if (mhi_cntrl->pm_state != MHI_PM_M0)
			dev_dbg(dev, "Return without waiting for M0\n");
