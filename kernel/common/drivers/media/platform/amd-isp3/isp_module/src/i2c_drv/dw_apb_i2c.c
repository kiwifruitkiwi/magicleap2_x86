...
int dw_i2c_disable(struct dw_device *dev)
{
	...
	while (dw_i2c_is_busy(dev)) {
		...
		if (isp_is_timeout(&start, &end, MAX_I2C_TIMEOUT)) {
			...
			ISP_PR_ERR("dev base address=0x%p\n",
					  dev->base_address);
			ISP_PR_ERR("reg=0x%08x\n", reg);

			ISP_PR_ERR("wait unbusy timeout(%dms)\n",
				MAX_I2C_TIMEOUT);
			return DW_ETIME;
		}
	};
	...
	while (dw_i2c_is_enabled(dev)) {
		isp_get_cur_time_tick(&end);
		if (isp_is_timeout(&start, &end, MAX_I2C_TIMEOUT)) {
			ISP_PR_ERR("wait disable timeout(%dms)\n",
				MAX_I2C_TIMEOUT);
...
int dw_i2c_set_speed_mode(struct dw_device *dev,
	enum dw_i2c_speed_mode mode)
{
	int retval;
	unsigned int reg;
	...
	if (mode > param->max_speed_mode)
		...
		if (DW_BIT_GET(reg, I2C_CON_SPEED) != (unsigned int) mode) {
...
enum dw_i2c_address_mode dw_i2c_get_master_address_mode(
...
enum dw_i2c_address_mode dw_i2c_get_slave_address_mode(
	struct dw_device *dev)
{
	unsigned int reg;
	enum dw_i2c_address_mode retval;
...
int dw_i2c_set_target_address(struct dw_device *dev,
...
unsigned short dw_i2c_get_target_address(struct dw_device *dev)
{
	unsigned int reg;
...
int dw_i2c_set_slave_address(struct dw_device *dev,
	unsigned short address)
{
	int retval;
	unsigned int reg;
...
unsigned short dw_i2c_get_slave_address(struct dw_device *dev)
{
	unsigned int reg;
	unsigned short retval;
	struct dw_i2c_portmap *portmap;
...
int dw_i2c_set_tx_mode(struct dw_device *dev,
	enum dw_i2c_tx_mode mode)
{
	int retval;
	unsigned int reg;
	struct dw_i2c_portmap *portmap;
	...
	if (dw_i2c_is_enabled(dev) == false) {
		...
		reg = I2C_INP(portmap->tar);
		/*avoid bus write if possible */
		if (DW_BIT_GET(reg, I2C_TAR_TX_MODE) != (unsigned int) mode) {
...
int dw_i2c_set_master_code(struct dw_device *dev,
	unsigned char code)
{
	int retval;
	unsigned int reg;
...
unsigned char dw_i2c_get_master_code(struct dw_device *dev)
{
	unsigned int reg;
	...
	retval = (unsigned char) DW_BIT_GET(reg, I2C_HS_MADDR_HS_MAR);
...
int dw_i2c_set_scl_count(struct dw_device *dev,
	enum dw_i2c_speed_mode mode, enum dw_i2c_scl_phase phase,
...
unsigned short dw_i2c_get_scl_count(struct dw_device *dev, enum
		dw_i2c_speed_mode mode, enum dw_i2c_scl_phase phase)
{
	...
	if (mode == i2c_speed_high) {
		...
		reg = 0;
		ISP_PR_ERR("illegal mode[%d]\n", mode);
	}

	retval = (unsigned short) DW_BIT_GET(reg, I2C_SCL_COUNT);
...
unsigned char dw_i2c_read(struct dw_device *dev)
{
	unsigned char retval;
	unsigned int reg;
	struct dw_i2c_portmap *portmap;
...
void dw_i2c_write(struct dw_device *dev, unsigned short character)
{
	struct dw_i2c_portmap *portmap;
...
void dw_i2c_issue_read(struct dw_device *dev,
	unsigned short stop, unsigned short restart)
{
	struct dw_i2c_portmap *portmap;
...
enum dw_i2c_tx_abort dw_i2c_get_tx_abort_source(
	struct dw_device *dev)
{
	unsigned int reg;
...
unsigned short dw_i2c_get_tx_fifo_depth(struct dw_device *dev)
{
	unsigned short retval;
	struct dw_i2c_param *param;
...
unsigned short dw_i2c_get_rx_fifo_depth(struct dw_device *dev)
{
	unsigned short retval;
	struct dw_i2c_param *param;
...
unsigned short dw_i2c_get_tx_fifo_level(struct dw_device *dev)
{
	unsigned int reg;
	unsigned short retval;
	struct dw_i2c_portmap *portmap;
	...
	retval = (unsigned short) DW_BIT_GET(reg, I2C_TXFLR_TXFL);
...
unsigned short dw_i2c_get_rx_fifo_level(struct dw_device *dev)
{
	unsigned int reg;
	unsigned short retval;
	struct dw_i2c_portmap *portmap;
	...
	retval = (unsigned short) DW_BIT_GET(reg, I2C_RXFLR_RXFL);
...
int dw_i2c_set_tx_threshold(struct dw_device *dev,
	unsigned char level)
{
	int retval;
	unsigned int reg;
...
unsigned char dw_i2c_get_tx_threshold(struct dw_device *dev)
{
	unsigned int reg;
	unsigned char retval;
	struct dw_i2c_portmap *portmap;
...
int dw_i2c_set_rx_threshold(struct dw_device *dev,
	unsigned char level)
{
	int retval;
	unsigned int reg;
...
unsigned char dw_i2c_get_rx_threshold(struct dw_device *dev)
{
	unsigned int reg;
	unsigned char retval;
	struct dw_i2c_portmap *portmap;
...
void dw_i2c_set_listener(struct dw_device *dev,
	dw_callback user_function)
{
	struct dw_i2c_portmap *portmap;
	struct dw_i2c_instance *instance;
...
int dw_i2c_master_back2_back(struct dw_device *dev,
	unsigned short *tx_buffer,
	unsigned int tx_length, unsigned char *rx_buffer,
	unsigned int rx_length, dw_callback user_function)
{
	...
	if (instance->state == i2c_state_idle) {
		...
		if (((unsigned long long) rx_buffer & 0x3) == 0)
			instance->rx_align = true;
		else
			instance->rx_align = false;

		/*support non-word aligned 16-bit buffers */
		tmp = (unsigned char *) tx_buffer;

		while (((((unsigned long long) tmp) & 0x3) != 0x0)
...
/*master transmit function*/
int dw_i2c_master_transmit(struct dw_device *dev,
...
int dw_i2c_slave_transmit(struct dw_device *dev,
	unsigned char *buffer,
	unsigned int length, dw_callback user_function)
{
	unsigned char *tmp;
...
int dw_i2c_slave_bulk_transmit(struct dw_device *dev,
	unsigned char *buffer,
	unsigned int length, dw_callback user_function)
{
	unsigned char *tmp;
...
int dw_i2c_master_receive(struct dw_device *dev,
			unsigned char *buffer,
			unsigned int length, dw_callback user_function)
{
	int retval;
	...
	if (instance->state == i2c_state_idle) {
		...
		if (((unsigned long long) buffer & 0x3) == 0)
			instance->rx_align = true;
		else
			instance->rx_align = false;
...
int dw_i2c_slave_receive(struct dw_device *dev,
			unsigned char *buffer,
			unsigned int length, dw_callback user_function)
{
	int retval;
...
void dw_i2c_unmask_irq(struct dw_device *dev,
			enum dw_i2c_irq interrupts)
{
	unsigned int reg;
	struct dw_i2c_portmap *portmap;
	struct dw_i2c_instance *instance;
	...
	if (((unsigned int) interrupts & reg) != (unsigned int) interrupts) {
		reg |= interrupts;
...
bool dw_i2c_is_irq_masked(struct dw_device *dev,
			enum dw_i2c_irq interrupt)
{
	int retval;
	unsigned int reg;
	struct dw_i2c_portmap *portmap;
...
unsigned int dw_i2c_get_irq_mask(struct dw_device *dev)
{
	unsigned int retval;
	struct dw_i2c_portmap *portmap;
...
bool dw_i2c_is_irq_active(struct dw_device *dev,
			enum dw_i2c_irq interrupt)
{
	bool retval;
	unsigned int reg;
...
bool dw_i2c_is_raw_irq_active(struct dw_device *dev,
			enum dw_i2c_irq interrupt)
{
	bool retval;
	unsigned int reg;
...
int dw_i2c_set_dma_tx_mode(struct dw_device *dev,
...
int dw_i2c_set_dma_rx_mode(struct dw_device *dev,
			enum dw_dma_mode mode)
{
	int retval;
	unsigned int reg;
...
int dw_i2c_set_dma_tx_level(struct dw_device *dev,
			unsigned char level)
{
	int retval;
	unsigned int reg;
...
unsigned char dw_i2c_get_dma_tx_level(struct dw_device *dev)
{
	unsigned int reg;
	unsigned int retval;
	struct dw_i2c_portmap *portmap;
...
int dw_i2c_set_dma_rx_level(struct dw_device *dev,
			unsigned char level)
{
	int retval;
	unsigned int reg;
...
unsigned char dw_i2c_get_dma_rx_level(struct dw_device *dev)
{
	unsigned int reg;
	unsigned int retval;
	struct dw_i2c_portmap *portmap;
...
int dw_i2c_irq_handler(struct dw_device *dev)
{
	...
	/*read request received */
		...
		case i2c_state_slave_bulk_tx_rx:
			 ...
			 *interrupt.
			 */
			max_bytes = MIN((unsigned int) param->tx_buffer_depth -
					(unsigned int)
					dw_i2c_get_tx_fifo_level(dev),
					instance->tx_remain);
...
int dw_i2c_flush_rx_hold(struct dw_device *dev)
{
	...
	if (instance->rx_idx != 4) {
		...
		if (instance->rx_remain >= 4) {
			buf = (unsigned int *) instance->rx_buffer;
			mask = ((unsigned int) (0xffffffff) >> (8 *
					instance->rx_idx));
			...
			*buf = (*buf & ~mask) | c;
			...
			tmp = (unsigned char *) instance->rx_buffer;
