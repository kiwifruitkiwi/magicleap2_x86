...
#include "SensorTdb_Dec31.h"

#include "AeTuningParam_Dec31.h"
#include "AwbTuningParam.h"
#include "PLineTuningParam.h"
#include "AfTuningParam.h"
#include "IridixTuningParam.h"

//Use for CAC new implement develop, default is disable
...
struct _CdbGainToBL_t {
	...
	struct _CdbBLCoeff_t bl[CDB_COMMON_MODULATION_NUM];
};

struct _PreBlackLevel_t {
	struct _CdbGainToBL_t blackLevel[CDB_PRE_BLS_CHANNEL_MAX_NUM];
};
...
struct _TdbDirGtf_t {
	unsigned int validSize;
	struct _TdbGainModulationUint8Coeff_t gTf[CDB_COMMON_MODULATION_NUM];
};

struct _TdbDirRbtf_t {
	unsigned int validSize;
	struct _TdbGainModulationUint8Coeff_t rbTf[CDB_COMMON_MODULATION_NUM];
};

struct _TdbDirItf_t {
	unsigned int validSize;
	struct _TdbGainModulationUint8Coeff_t rTf[CDB_COMMON_MODULATION_NUM];
};

struct _TdbDirDpuTh_t {
	unsigned int validSize;
	struct _TdbGainModulationInt16Coeff_t dpuTh[CDB_COMMON_MODULATION_NUM];
};

struct _TdbDirDpaTh_t {
	unsigned int validSize;
	struct _TdbGainModulationInt16Coeff_t dpaTh[CDB_COMMON_MODULATION_NUM];
};

struct _TdbDirRgbTh_t {
	unsigned int validSize;
	struct _TdbGainModulationUint16Coeff_t rgbTh[CDB_COMMON_MODULATION_NUM];
};

struct _TdbDirIrTh_t {
	unsigned int validSize;
	struct _TdbGainModulationUint16Coeff_t irTh[CDB_COMMON_MODULATION_NUM];
};

struct _TdbDirDiff_t {
	unsigned int validSize;
	struct _TdbGainModulationUint8Coeff_t diff[CDB_COMMON_MODULATION_NUM];
};

struct _TdbDirCdAth_t {
	unsigned int validSize;
	struct _TdbGainModulationInt16Coeff_t cdAth[CDB_COMMON_MODULATION_NUM];
};

struct _TdbDirCdUth_t {
	unsigned int validSize;
	struct _TdbGainModulationInt16Coeff_t cdUth[CDB_COMMON_MODULATION_NUM];
};
...
struct _TdbZzhdrItimeDiffParams_t {
	...
	//range:0-64     if sigSatNlsnr=0, Low>=High; else Low <= High
	unsigned char                   bldSatWgtLow;
	...
	//range:0-64   if sigNsatNlsnrMot=0 Low>=High; else Low <= High
	unsigned char                   bldMotWgtLow;
...
struct _TdbZzhdr_t {
	...
	//u1.0    must set sigSatNlsnr =  sigSatLsnr
	unsigned char                   sigSatNlsnr;
...
struct _CdbShadingRadial_t {
	struct _CdbShadingRadialCoeff_t shading[CDB_3CH_COLOR_COMPONENT_MAX];
};
...
struct _CdbRadialShading_t {
	struct _CdbShadingRadial_t      shadingRadial;
	struct _CdbShadingRadialCentreAndMult_t shadingRadialCentreAndMult;
};
...
struct _CdbBlackLevel_t {
	struct _CdbGainToBL_t blackLevel[CDB_4CH_COLOR_COMPONENT_MAX];
};
...
struct _CdbAbsoluteCcm_t {
	unsigned int validSize;
	struct _CdbCcmCoeff_t ccm[CDB_CCM_COMPONENT_MAX];
};
...
struct _CdbMeshShadingLUT_t {
	unsigned int validSize;
	struct _CdbShadingChannel_t shadingChannel[CDB_SHADING_COMPONENT_MAX];
};

struct _CdbMeshShadingStrength_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t strength[CDB_COMMON_MODULATION_NUM];
};

// @ModuleId: CALIB_COREPIPE_MESH_SHADING
struct _CdbShading_t {
	unsigned short cctThreshold[CDB_SHADING_CCT_TH_NUM];
	unsigned char meshAlphaMode;
	unsigned char meshScale;
	unsigned char meshWidth;
	unsigned char meshHeight;
	struct _CdbMeshShadingStrength_t        meshShadingStrength;
	struct _CdbMeshShadingLUT_t             meshShadingLut;
};
...
//The sinter and temper only has 32 lut registers, but calib data has 128
//params. Each register should set 4 values to 4 fields.
...
struct _CdbGamma_t {
	...
	unsigned short gammaCurveEv1[129];
	unsigned short gammaCurveEv2[129];
};
...
struct _CdbCaCorrectionMem_t {
	...
	unsigned int validSize;
	struct _CdbCaCorrectionMemCoeff_t caMemCoeff[CDB_CA_MEM_NUM];
#else
...
struct _CdbSinterStrength4_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t strength4[CDB_COMMON_MODULATION_NUM];
};

struct _CdbSinterStrength1_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t strength1[CDB_COMMON_MODULATION_NUM];
};

struct _CdbSinterThresh1H_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t thresh1H[CDB_COMMON_MODULATION_NUM];
};

struct _CdbSinterThresh1V_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t thresh1V[CDB_COMMON_MODULATION_NUM];
};

struct _CdbSinterThresh4H_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t thresh4H[CDB_COMMON_MODULATION_NUM];
};

struct _CdbSinterThresh4V_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t thresh4V[CDB_COMMON_MODULATION_NUM];
};

struct _CdbSinterIntconfig_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t intconfig[CDB_COMMON_MODULATION_NUM];
};

struct _CdbSinterSad_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t sad[CDB_COMMON_MODULATION_NUM];
};
...
struct _CdbSharpAltD_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t altD[CDB_COMMON_MODULATION_NUM];
};

struct _CdbSharpAltUd_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t altUd[CDB_COMMON_MODULATION_NUM];
};

struct _CdbSharpAltDu_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t altDu[CDB_COMMON_MODULATION_NUM];
};
...
struct _CdbDemosaicNpOffset_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t npOffset[CDB_COMMON_MODULATION_NUM];
};

struct _CdbDemosaicUuSlope_t {
	unsigned int validSize;
	struct _CdbGainModulationU8Coeff_t uuSlop[CDB_COMMON_MODULATION_NUM];
};

struct _CdbDemosaicSharpAltLu_t {
	unsigned int validSize;
	struct _CdbGainModulationU8Coeff_t altLu[CDB_COMMON_MODULATION_NUM];
};

struct _CdbDemosaicSharpAltLdu_t {
	unsigned int validSize;
	struct _CdbGainModulationU8Coeff_t altLdu[CDB_COMMON_MODULATION_NUM];
};

struct _CdbDemosaicSharpAltLd_t {
	unsigned int validSize;
	struct _CdbGainModulationU8Coeff_t altLd[CDB_COMMON_MODULATION_NUM];
};
...
struct _CdbDemosaicFcSlop_t {
	unsigned int validSize;
	struct _CdbGainModulationU8Coeff_t fcSlop[CDB_COMMON_MODULATION_NUM];
};
...
struct  _CdbIridix8StrengthDkEnhCtl_t {
	...
	// pD_cut_max: maximum intensity cut for dark regions
	// in which dk_enh will be applied
	unsigned int	pdCutMax;
	unsigned int	darkContrastMin;// dark contrast min
	unsigned int	darkContrastMax;// dark contrast max
	// min_str: iridix strength in percentage
	unsigned int	minStr;
	// max_str: iridix strength in percentage: 50 = 1x gain. 100 = 2x gain
	unsigned int	maxStr;
	// dark_prc_gain_target: target in histogram (percentage) for
	// dark_prc after iridix is applied
...
struct _CdbDpSlop_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t slop[CDB_COMMON_MODULATION_NUM];
};

struct _CdbDpThreshold_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t threshold[CDB_COMMON_MODULATION_NUM];
};
...
struct _CdbStitchingLMMedNoiseIntensityThresh_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t thresh[CDB_COMMON_MODULATION_NUM];
};

struct _CdbStitchingLmMovMult_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t lmMovMult[CDB_COMMON_MODULATION_NUM];
};

struct _CdbStitchingMsMovMult_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t msMovMult[CDB_COMMON_MODULATION_NUM];
};

struct _CdbStitchingSvsMovMult_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t svsMovMult[CDB_COMMON_MODULATION_NUM];
};

struct _CdbStitchingLmNp_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t lmNp[CDB_COMMON_MODULATION_NUM];
};

struct _CdbStitchingMsNp_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t msNp[CDB_COMMON_MODULATION_NUM];
};

struct _CdbStitchingSvsNp_t {
	unsigned int validSize;
	struct _CdbGainModulationCoeff_t svsNp[CDB_COMMON_MODULATION_NUM];
};

// @ModuleId: CALIB_COREPIPE_STITCHING
struct _CdbStitching_t {
	struct _CdbStitchingLMMedNoiseIntensityThresh_t lmMedNoiseIntensityTh;
...
struct _CdbCalibraitonAeControl_t {
	...
	unsigned int irGdgEnable;// 1:0 enable | disable iridix global gain.
...
struct _CdbCustSettings_t {
	unsigned int validSize;
	struct _CdbCustSettingsValue_t     custSetting[CDB_CUST_SETTINGS_NUM];
};
#endif
...
struct _CdbTopCfg_t {
	...
	int bypassInputFormatter;
	int bypassDecompander;
	int bypassSensorOffsetWd;
	int bypassGainWdr;
	int bypassFrameStitch;
	int byPassDigitalGain;
	int byPassDefectPixel;
	int byPassGE;
	int byPassDPC;
	int byPassCaCorrection;
	int byPassTemper;
	int byPassSinter;
	int byPassMeshShading;
	int byPassRadialShading;
	int byPassWhiteBalance;
	int byPassIridix;
	int byPassIridixGain;
	int bypassDemosaic;
	int byPassPfCorrection;
	int byPassCCM;
	int bypass3DLUT;
	int bypassNonequGamma;
	int bypassFrRgbGamma;
	int bypassFrShapen;
	int bypassCsConv;
};
...
struct _CorePipeCdb_t {
	...
	struct _CdbWdrNoiseProfile_t            wdrNp;	//stitch niose profile
	...
	struct _CdbGamma_t                      gamma;//fr rgb gamma
	...
	struct _CdbCalibraitonAeControl_t       aeControl;//for iridix
#ifdef ENABLE_SCR
...
//Notice: this struct should align with enum _IspPipeOutCh_t in ParamsType.h
...
struct _YuvResize_t {
	...
	struct _YuvResizeTable_t table[CDB_ISP_PIPE_OUT_CH_MAX - 1];
};
...
struct _TdbYuvNrYByGain_t {
	unsigned int validSize;
	struct _TdbGainModulationYuvNrY_t params[CDB_COMMON_MODULATION_NUM];
};

struct _TdbYuvNrUvByGain_t {
	unsigned int validSize;
	struct _TdbGainModulationYuvNrUv_t params[CDB_COMMON_MODULATION_NUM];
};
...
struct _TdbSharpRegByRatio_t {
	unsigned int validSize;
	struct _TdbRatioModulationSharp_t params[CDB_COMMON_MODULATION_NUM];
};
...
struct _TdbSharpRegByGain_t {
	unsigned int validSize;
	struct _TdbGainModulationSharp_t params[CDB_COMMON_MODULATION_NUM];
};
...
struct _SceneModePipeDelta_t {
	struct _CdbSinter_t sinter[SCENE_PREFERENCE_TUNING_PIPE_SETS_NUM];
	struct _CdbTemper_t temper[SCENE_PREFERENCE_TUNING_PIPE_SETS_NUM];
	struct _TdbYuvNr_t yuvNr[SCENE_PREFERENCE_TUNING_PIPE_SETS_NUM];
...
struct _MappingAaa_t {
	struct _SceneMappingAaa_t sceneMapping[SCENE_MODE_MAX];
};
struct _SensorTdb_t {
	unsigned int toolVersion;
	unsigned int tdbVersion;
	...
	struct _PostPipeTdb_t postPipeTdb;
#ifndef CONFIG_DISABLE_TDB_NEW_ELEMENT
	struct _AeCdb_t aeCdb[SCENE_PREFERENCE_TUNING_AAA_SETS_NUM];
	struct _PLineTable_t plineCdb[SCENE_PREFERENCE_TUNING_AAA_SETS_NUM];
	struct _AwbCdb_t awbCdb[SCENE_PREFERENCE_TUNING_AAA_SETS_NUM];
	struct _AfCdb_t afCdb[SCENE_PREFERENCE_TUNING_AAA_SETS_NUM];
	struct _IridixCdb_t iridixCdb[SCENE_PREFERENCE_TUNING_AAA_SETS_NUM];
	struct _MappingAaa_t mappingAaa[SCENARIO_MODE_MAX];
#endif
	// CONFIG_DISABLE_TDB_NEW_ELEMENT
