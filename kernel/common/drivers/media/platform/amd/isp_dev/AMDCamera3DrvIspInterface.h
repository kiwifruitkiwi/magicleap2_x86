/**
 * @file  AMDCamera3DrvIspInterface.h
 * @brief Header file for interface with V4L2 driver
 **/
...
typedef enum _ispFlickerPeriod_t {
	ISP_FLICKER_OFF,
	ISP_FLICKER_100HZ,
	ISP_FLICKER_120HZ
} ispFlickerPeriod_t;

typedef enum _isoValue_t {
	ISO_VALUE_AUTO = 0,
	ISO_VALUE_100 = 1,
	ISO_VALUE_200 = 2,
	ISO_VALUE_400 = 3,
...
typedef enum _flashlightMode_t {
...
typedef struct _ispEvValue_t {
	int32_t valueNumerator;
	int32_t valueDenominator;
} ispEvValue_t;

typedef struct _metaDataAf_t {
	ispAfStatus_t afStatus;// value is from enum af_status
	float sharpness[3]; // AF sharpness value
	uint32_t lensPos; // Lens pos gotten from AF
	uint32_t afValidBits; // Ored META_VALID_BIT_XX
	ispAfMode_t afMode;// afMode, indicated by META_VALID_BIT_AF_MODE
} metaDataAf_t;

typedef struct _metaDataAec_t {
	ispAecStatus_t aecStatus; // value is from enum aec_status
	...
	isoValue_t iso; // value is from enum iso_value
	...
	ispAecMode_t aecMode; // aeExposureCompensation,
				// indicated by META_VALID_BIT_AE_MODE
	ispFlickerPeriod_t freq; // aeAntibandingMode,
				// indicated by META_VALID_BIT_FREQ
	ispEvValue_t ev; // aeExposureCompensation,
			// indicated by META_VALID_BIT_EV
} metaDataAec_t;

typedef struct _metaDataAwb_t {
	ispAwbStatus_t awbStatus;// value is from enum aec_status
	...
	ispAwbMode_t awbMode;// indicated by META_VALID_BIT_AWB_MODE
} metaDataAwb_t;

typedef struct _metaDataFlash_t {
	flashStatus_t flashStatus; // value is from enum flash_status
	uint32_t flashValidBits; // Ored META_VALID_BIT_XX
	flashlightMode_t flMode; // flash mode,
				 // indicated by META_VALID_BIT_FL_MODE
} metaDataFlash_t;

typedef struct _metaDataInfo_t {
	ispSensorId_t sensorId; //the sensor that the meta data corresponds to
	uint32_t poc;
	uint32_t timeStamp;
	metaDataAf_t metaAf;
	metaDataAec_t metaAec;
	metaDataAwb_t metaAwb;
	metaDataFlash_t metaFlash;
