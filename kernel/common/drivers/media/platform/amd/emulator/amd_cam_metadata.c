...
static struct tag_info
	...
	AMD_CAM_VENDOR_CONTROL_START] = {
"forceapply_supported_modes", TYPE_INT32},
	[AMD_CAM_VENDOR_CONTROL_EFFECT_MODE - AMD_CAM_VENDOR_CONTROL_START] = {
		"effectmode", TYPE_BYTE},
	...
	AMD_CAM_VENDOR_CONTROL_START] = {
"app_exposure_time_upper_limit", TYPE_INT64},
	...
	AMD_CAM_VENDOR_CONTROL_START] = {
"lens_control_available_modes", TYPE_BYTE},
};
		...
		u32 tag, u32 type, const void *data, size_t data_count)
{
	...
	ret = clear_user(entry, sizeof(struct amd_cam_metadata_buffer_entry));
	if (ret) {
		pr_err("%s: clearing user space buffer failed %lu\n", __func__,
		       ret);
		return -EINVAL;
	}
	...
	if (data_bytes == 0) {
		copied =
		    copy_to_user(entry->data.value, data, data_payload_bytes);
		if (copied) {
			pr_err("%s:%d: copy_to_user failed with %d\n", __func__,
			       __LINE__, copied);
			return -EINVAL;
		}
	} else {
		...
		copied =
		    copy_to_user((amd_cam_get_data(dst) + entry->data.offset),
		...
		struct amd_cam_metadata_entry *updated_entry)
{
	...
	if (data_bytes != entry_bytes) {
		...
		if (data_bytes != 0) {
			...
			copied =
			    copy_to_user((amd_cam_get_data(dst) +
