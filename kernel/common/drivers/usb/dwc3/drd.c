				    ...
				    enum usb_role role)
{
	...
	dev_info(dwc->dev, "role switch to %s\n",
		 mode == DWC3_GCTL_PRTCAP_HOST ? "HOST" : "DEVICE");
...
static const struct property_entry amd_dwc3_props[] = {
...
static int dwc3_setup_role_switch(struct dwc3 *dwc)
{
	...
	dwc3_set_mode(dwc, mode);
	dev_info(dwc->dev, "role switch default set to %s\n",
		 mode == DWC3_GCTL_PRTCAP_HOST ? "HOST" : "DEVICE");
	return 0;
}
#else
