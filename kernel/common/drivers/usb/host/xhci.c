...
static inline bool xhci_port_valid(const struct xhci_hub *rhub, int port1)
{
	/* Port Index is 1-based */
	if (port1 < 1)
		return false;

	if (port1 > rhub->num_ports)
		return false;

	if (!rhub->ports[port1 - 1])
		return false;
...
int xhci_find_raw_port_number(struct usb_hcd *hcd, int port1)
{
	...
	if (!xhci_port_valid(rhub, port1)) {
		xhci_err(hcd_to_xhci(hcd), "Invalid port: %d\n", port1);
		return -EINVAL;
	}
