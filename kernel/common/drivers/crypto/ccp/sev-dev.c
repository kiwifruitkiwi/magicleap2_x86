...
static int sev_ioctl_do_pek_csr(struct sev_issue_cmd *argp)
{
	...
	if (!access_ok((void *)input.address, input.length) ||
...
static int sev_ioctl_do_get_id2(struct sev_issue_cmd *argp)
{
	...
	if (input.address &&
	    input.length &&
	    !access_ok((void *)input.address, input.length))
		return -EFAULT;
...
static int sev_ioctl_do_pdh_export(struct sev_issue_cmd *argp)
{
	...
	if ((input.pdh_cert_len > SEV_FW_BLOB_MAX_SIZE) ||
	    !access_ok((void *)input.pdh_cert_address, input.pdh_cert_len)) {
		ret = -EFAULT;
		goto e_free;
	}
	...
	if ((input.cert_chain_len > SEV_FW_BLOB_MAX_SIZE) ||
	    !access_ok((void *)input.cert_chain_address, input.cert_chain_len)) {
		ret = -EFAULT;
		goto e_free;
	}
