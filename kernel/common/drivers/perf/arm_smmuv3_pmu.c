...
#define SMMU_PMCG_OFFSET                0x2000
...
static void smmu_pmu_event_update(struct perf_event *event)
{
	...
	dev_dbg(smmu_pmu->dev, "event-idx=%d\n", idx);
	do {
				...
				struct hw_perf_event *hwc)
{
	...
	dev_dbg(smmu_pmu->dev, "event-idx=%d, counter=%lld\n", idx, new);
				       ...
				       struct perf_event *event, int idx)
{
	...
	dev_err(smmu_pmu->dev, "failed apply event filter\n");
	return -EAGAIN;
}
				  ...
				  struct perf_event *event)
{
	...
	if (idx == num_ctrs) {
		dev_err(smmu_pmu->dev, "failed obtain unused counter\n");
...
static int smmu_pmu_event_init(struct perf_event *event)
{
	...
	if (event->attr.type != event->pmu->type) {
		dev_dbg(dev, "attr.type:%x != pmu->type:%x ***\n",
	...
	if (event_id < SMMU_PMCG_ARCH_MAX_EVENTS &&
		...
		dev_err(dev, "Invalid event %d for this PMU\n", event_id);
		return -EINVAL;
	}
	...
	if (!is_software_event(event->group_leader)) {
		if (!smmu_pmu_events_compatible(event->group_leader, event)) {
			dev_err(dev, "event not compatibled\n");
			return -EINVAL;
		}

		if (++group_num_events > smmu_pmu->num_counters) {
			dev_err(dev, "group event more than counters\n");
			return -EINVAL;
		}
	}
	...
	for_each_sibling_event(sibling, event->group_leader) {
		...
		if (!smmu_pmu_events_compatible(sibling, event)) {
			dev_err(dev, "sibling event not compatibled\n");
			return -EINVAL;
		}

		if (++group_num_events > smmu_pmu->num_counters) {
			dev_err(dev, "sibling group event more than counters\n");
			return -EINVAL;
		}
	}
					 ...
					 struct attribute *attr, int unused)
{
	...
	dev_dbg(dev, "ID:%llx not found in supported_events ***\n",
		pmu_attr->id);
	return 0;
}
...
static irqreturn_t smmu_pmu_irq_thread(int irq_num, void *data)
{
...
static int smmu_pmu_setup_irq(struct smmu_pmu *pmu)
{
	...
	irq = pmu->irq;
	if (irq)
		ret = devm_request_threaded_irq(pmu->dev, irq,
						smmu_pmu_handle_irq,
						smmu_pmu_irq_thread,
...
static int smmu_pmu_probe(struct platform_device *pdev)
{
	...
	node = of_parse_phandle(pdev->dev.of_node, "SMMU_ADDR", 0);
	if (!node) {
		node = pdev->dev.of_node;
		dev_dbg(dev, "cannot find arm smmu device. assume TBU.\n");
	}

	res_map = of_iomap(node, 0);
	if (IS_ERR(res_map)) {
		dev_err(dev, "Missing resource %d\n", 0);
		return PTR_ERR(res_map);
	}

	/* PMCG registers offset from SMMUv3 page 0 base by 0x2000 */
	smmu_pmu->reg_base = res_map + SMMU_PMCG_OFFSET;
	of_address_to_resource(node, 0, &res_0);

	if (of_property_read_u32_index(pdev->dev.of_node, "override_cfg", 0,
				       &cfgr))
		cfgr = readl_relaxed(smmu_pmu->reg_base + SMMU_PMCG_CFGR);
	else
		dev_dbg(dev, "override SMMU PMCG config:0x%x\n", cfgr);

	/* Determine if page 1 is present */
	if (cfgr & SMMU_PMCG_CFGR_RELOC_CTRS) {
		dev_dbg(dev, "PMCG page1 offset will be used instead\n");
		smmu_pmu->reloc_base = of_iomap(node, 1);
		if (IS_ERR(smmu_pmu->reloc_base)) {
			dev_err(dev, "Missing resource 1 for PMCG page 1\n");
	...
	if (of_property_read_u64_index(pdev->dev.of_node, "override_ceid", 0,
				       &ceid_64[0]))
		ceid_64[0] = readq_relaxed(smmu_pmu->reg_base + SMMU_PMCG_CEID0);
	else
		dev_dbg(dev, "override SMMU PMCG ceid0:0x%llx\n", ceid_64[0]);

	if (of_property_read_u64_index(pdev->dev.of_node, "override_ceid", 1,
				       &ceid_64[1]))
		ceid_64[1] = readq_relaxed(smmu_pmu->reg_base + SMMU_PMCG_CEID1);
	else
		dev_dbg(dev, "override SMMU PMCG ceid1:0x%llx\n", ceid_64[1]);

	bitmap_from_arr32(smmu_pmu->supported_events, (u32 *)ceid_64,
			  SMMU_PMCG_ARCH_MAX_EVENTS);
	dev_dbg(dev, "** SMMU pmu supported events, ceid0:0x%llx, ceid1:0x%llx\n",
		ceid_64[0], ceid_64[1]);
	...
	name = devm_kasprintf(&pdev->dev, GFP_KERNEL, "smmuv3_pmcg_%llx",
			      ((long long)res_0.start) >> SMMU_PMCG_PA_SHIFT);
	if (!name) {
	...
	dev_dbg(dev, "Registered PMU %s using %d counters with %s filter settings\n",
		name, smmu_pmu->num_counters,
...
core_initcall(arm_smmu_pmu_init);
