...
char cnss_wfc_call_status_debug_write_buf[(QMI_WLFW_MAX_WFC_CALL_STATUS_DATA_SIZE_V01 + 1) * 5];
						...
						size_t count, loff_t *off)
{
	...
	len = min(count, sizeof(cnss_wfc_call_status_debug_write_buf) - 1);
	if (copy_from_user(cnss_wfc_call_status_debug_write_buf, user_buf, len))
		return -EFAULT;

	cnss_wfc_call_status_debug_write_buf[len] = '\0';
	sptr = cnss_wfc_call_status_debug_write_buf;
