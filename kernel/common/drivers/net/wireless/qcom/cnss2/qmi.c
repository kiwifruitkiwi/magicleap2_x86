...
int cnss_wlfw_respond_mem_send_sync(struct cnss_plat_data *plat_priv)
{
	...
	for (i = 0; i < req->mem_seg_len; i++) {
		...
		cnss_pr_dbg("Memory for FW, va: 0x%p, pa: %pa, size: 0x%zx, type: %u\n",
				  ...
				  enum cnss_driver_mode mode)
{
	...
	if (mode == CNSS_OFF &&
		...
		cnss_pr_dbg("Idle shutdown is in progress, ignore mode off request\n");
		return 0;
	}
...
int cnss_qmi_init(struct cnss_plat_data *plat_priv)
{
	...
	memset(plat_priv->remote_mem, 0,
	       sizeof(struct fw_remote_mem) * BHI_WLFW_MAX_NUM_MEM_SEG_V01);
