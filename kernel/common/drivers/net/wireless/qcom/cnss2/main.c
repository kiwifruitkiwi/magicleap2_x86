...
static char modp_wlan_mac[IFHWADDRLEN * 3];
module_param_string(wlan_mac, modp_wlan_mac, sizeof(modp_wlan_mac), 0644);
MODULE_PARM_DESC(wlan_mac, "WLAN MAC address");
...
uint8_t *cnss_get_custom_wlan_mac_address(struct device *dev, uint32_t *num)
{
	struct cnss_plat_data *plat_priv = cnss_bus_dev_to_plat_priv(dev);
	...
	*num = 1;
	return plat_priv->wlan_mac_addr;
}
EXPORT_SYMBOL(cnss_get_custom_wlan_mac_address);
...
{
	cnss_subsys_crash_shutdown(NULL);
...
{
	plat_priv->ctrl_params.quirks = quirks ? quirks : CNSS_QUIRKS_DEFAULT;
...
static void cnss_set_wlan_mac_addr(struct cnss_plat_data *plat_priv)
{
	const u8 ml_wlan_mac_addr[IFHWADDRLEN] = {0x60, 0x4b, 0xaa, 0, 0, 0};
	char *macptr = modp_wlan_mac;
	int hwaddrlen = 0;
	char *token;

	while ((token = strsep(&macptr, ":")) != NULL &&
	       hwaddrlen < IFHWADDRLEN) {
		u8 val;

		if (kstrtou8(token, 0x10, &val) < 0)
			break;
		plat_priv->wlan_mac_addr[hwaddrlen++] = val;
	}

	if (hwaddrlen != IFHWADDRLEN) {
		uint rand_mac;

		cnss_pr_info("generate ramdon MAC!\n");
		prandom_seed((uint)jiffies);
		rand_mac = prandom_u32();
		memcpy(plat_priv->wlan_mac_addr, ml_wlan_mac_addr,
		       sizeof(plat_priv->wlan_mac_addr));
		plat_priv->wlan_mac_addr[3] = (u8)rand_mac;
		plat_priv->wlan_mac_addr[4] = (u8)(rand_mac >> 8);
		plat_priv->wlan_mac_addr[5] = (u8)(rand_mac >> 16);
	}

	cnss_pr_info("MAC addr = %02x:%02x:%02x:%02x:%02x:%02x\n",
		     plat_priv->wlan_mac_addr[0], plat_priv->wlan_mac_addr[1],
		     plat_priv->wlan_mac_addr[2], plat_priv->wlan_mac_addr[3],
		     plat_priv->wlan_mac_addr[4], plat_priv->wlan_mac_addr[5]);
}
...
static int cnss_probe(struct platform_device *plat_dev)
{
	...
	of_id = of_match_device(cnss_of_match_table, dev);
	if (!of_id || !of_id->data) {
	...
	plat_priv = devm_kzalloc(dev, sizeof(*plat_priv), GFP_KERNEL);
	if (!plat_priv) {
		ret = -ENOMEM;
		goto out;
	}
	...
	plat_priv->plat_dev = plat_dev;
	/* WL_EN is an optional device resource */
	plat_priv->gpio_wl_en = devm_gpiod_get(dev, "reset", GPIOD_OUT_HIGH);
	if (IS_ERR(plat_priv->gpio_wl_en)) {
		cnss_pr_warn("Unable to obtain wl_en gpio, %ld\n",
			     PTR_ERR(plat_priv->gpio_wl_en));
	...
	/* Do not require Qualcomm's msm PCIe driver for suspend/resume */
	plat_priv->ctrl_params.quirks |= BIT(DISABLE_DRV);

	cnss_set_wlan_mac_addr(plat_priv);
...
module_platform_driver(cnss_platform_driver);
