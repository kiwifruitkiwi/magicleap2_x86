...
#define MHI_TIMEOUT_OVERWRITE_MS	90000
...
struct mhi_controller *mhi_alloc_controller(size_t size){
	return kzalloc(sizeof(struct mhi_controller), GFP_KERNEL);
}
void mhi_dump_sfr(struct mhi_controller *mhi_cntrl){return;}
void mhi_debug_reg_dump(struct mhi_controller *mhi_cntrl){return;}
...
retry:
		...
		if (pci_priv->pci_dev->current_state != PCI_D0 &&
		    retry++ < POWER_ON_RETRY_MAX_TIMES) {
			cnss_pr_dbg("Retry to set power state #%d\n",
				    retry);
...
void cnss_pci_force_wake(struct cnss_pci_data *pci_priv)
{
        mhi_reg_write_remap(pci_priv,
                                 pci_priv->bar,
			PCIE_SOC_WAKE_PCIE_LOCAL_REG, 1);
        mdelay(5);
}
				     ...
				     enum mhi_callback reason)
{
	...
	else if (reason == MHI_CB_BOOTUP_TIMEOUT)
		cnss_pci_dump_mhi_reg(pci_priv);
}
...
#endif
#ifdef CONFIG_X86
        mhi_ctrl->rddm_size = 0x420000;
        pr_err("rddm size %lx",mhi_ctrl->rddm_size);
#else
...
void cnss_pci_shutdown(struct pci_dev *pdev)
{
	...
	if (pci_priv->plat_priv->powered_on) {
		cnss_pci_sw_reset(pdev, false);
		cnss_power_off_device(pci_priv->plat_priv);
	}
}
