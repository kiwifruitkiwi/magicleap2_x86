 ...
 *
 * $Id: dhd_linux.c 785540 2018-10-19 09:14:18Z $
 */
...
#ifdef DHD_4WAYM4_FAIL_DISCONNECT
extern msg_4way_t dhd_is_4way_msg(void *pbody);
#endif /* DHD_4WAYM4_FAIL_DISCONNECT */
...
#endif /* DHD_8021X_DUMP */
#ifdef DHD_4WAYM4_FAIL_DISCONNECT
			if (dhd_is_4way_msg((char *)eh + ETHER_HDR_LEN) == M3_4WAY) {
...
write_dump_to_file(dhd_pub_t *dhd, uint8 *buf, int size, char *fname)
{
	...
	snprintf(memdump_path, sizeof(memdump_path), "%s%s_%s_" "%s",
		"/data/misc/wifi/", fname, memdump_type,  dhd->debug_dump_time_str);
...
#define MEMDUMPINFO_LIVE MEMDUMPINFO
#define MEMDUMPINFO_INST MEMDUMPINFO
...
get_debug_dump_time(char *str)
{
	...
	if (!strlen(str)) {
		...
		snprintf(str, DEBUG_DUMP_TIME_BUF_LEN, DHD_LOG_DUMP_TS_FMT_YYMMDDHHMMSSMSMS,
				...
				tm.tm_sec, (int)(curtime.tv_nsec/NSEC_PER_USEC));
	}
}
...
msg_4way_t
dhd_is_4way_msg(void *pbody)
{
	eapol_hdr_t* eapol = (eapol_hdr_t*)pbody;
...
dhd_eap_txcomplete(dhd_pub_t *dhdp, void *txp, bool success, int ifidx)
{
	...
	if (!success) {
		...
		type  = ntoh16(eh->ether_type);
		if (type == ETHER_TYPE_802_1X) {
			if (dhd_is_4way_msg((char *)eh + ETHER_HDR_LEN) == M4_4WAY) {
				dhd_if_t *ifp = NULL;
				ifp = dhd->iflist[ifidx];
				if (!ifp || !ifp->net) {
					return;
