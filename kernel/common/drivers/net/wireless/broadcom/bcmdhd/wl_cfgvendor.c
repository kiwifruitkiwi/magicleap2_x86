 ...
 *
 * $Id: wl_cfgvendor.c 785223 2018-10-17 17:51:07Z $
 */
...
int wl_cfgvendor_send_port_authorized_event(struct wiphy *wiphy, struct net_device *dev,
        const struct ether_addr *peer_addr)
{
	...
	LINUX_VERSION_CODE >= KERNEL_VERSION(4, 1, 0)
	skb = cfg80211_vendor_event_alloc(wiphy, ndev_to_wdev(dev),
		BRCM_VENDOR_EVENT_BUF_LEN, BRCM_VENDOR_EVENT_PORT_AUTHORIZED, kflags);
#else
	skb = cfg80211_vendor_event_alloc(wiphy, BRCM_VENDOR_EVENT_BUF_LEN,
		BRCM_VENDOR_EVENT_PORT_AUTHORIZED, kflags);
#endif /* (defined(CONFIG_ARCH_MSM) && defined(SUPPORT_WDEV_CFG80211_VENDOR_EVENT_ALLOC)) || */
	/* LINUX_VERSION_CODE >= KERNEL_VERSION(4, 1, 0) */
	if (!skb) {
		WL_ERR(("skb alloc failed"));
	...
	cfg80211_vendor_event(skb, kflags);
	WL_INFORM_MEM(("port authorized event sent\n"));
	return BCME_OK;
}
