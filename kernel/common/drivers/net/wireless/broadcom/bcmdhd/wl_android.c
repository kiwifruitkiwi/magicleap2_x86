 ...
 *
 * $Id: wl_android.c 785384 2018-10-18 08:08:13Z $
 */
...
_wl_android_bcnrecv_start(struct bcm_cfg80211 *cfg, struct net_device *ndev, bool user_trigger)
{
	...
	if (wl_get_drv_status(cfg, SCANNING, ndev) ||
		...
		err = BCME_BUSY;
		WL_ERR(("Scan in progress, Aborting the scan error:%d\n", err));
		goto exit;
	}
...
static s32
wl_cfg80211_handle_cac_action(struct bcm_cfg80211 * cfg, struct net_device * ndev, char * argv)
{
	...
	if (strncmp(argv, "addts", strlen("addts")) == 0) {
		...
		sscanf(argv, "%*s %hhu %hhu %hhu %s %hu %hu", &tspec_arg.dialog_token,
			&user_tid, &user_prio, user_argv,
			&tspec_arg.nom_msdu_size, &tspec_arg.surplus_bw);
		if ((user_tid > TSPEC_MAX_USER_TID) || (user_prio > TSPEC_MAX_USER_PRIO) ||
			(tspec_arg.dialog_token == 0) ||
			...
			WL_ERR(("Invalid params user_tid %hhu user_prio %hhu dialog_token %hhu"
				" nom_msdu_size %hu surplus BW %hu\n", user_tid, user_prio,
				tspec_arg.dialog_token, tspec_arg.nom_msdu_size,
				tspec_arg.surplus_bw));
			return BCME_USAGE_ERROR;
		}

		user_tid <<= TSPEC_TID_SHIFT;
		user_prio <<= TSPEC_UP_SHIFT;
		/* Using U-APSD as the power save mode  for now */
		user_psb = TSPEC_UAPSD_PSB;
		/* Update the direction based on the params passed */
		if (!strncmp(user_argv, "uplink", strlen("uplink"))) {
			direction = TSPEC_UPLINK_DIRECTION;
		} else if (!strncmp(user_argv, "downlink", strlen("downlink"))) {
			direction = TSPEC_DOWNLINK_DIRECTION;
		} else if (!strncmp(user_argv, "bi-directional", strlen("bi-directional"))) {
			direction = TSPEC_BI_DIRECTION;
		} else {
			WL_ERR(("Invalid direction param\n"));
			return BCME_USAGE_ERROR;
		}

		tspec_arg.tsinfo.octets[0] = (uint8)(user_tid |
			direction | TSPEC_EDCA_ACCESS);
		...
		tspec_arg.version = TSPEC_ARG_VERSION;
		/* Read the params passed */
		sscanf(argv, "%*s %hhu %hhu %s", &user_tid, &user_prio, user_argv);

		if ((user_tid > TSPEC_MAX_USER_TID) || (user_prio > TSPEC_MAX_USER_PRIO)) {
			WL_INFORM_MEM(("%hhu %hhu \n", user_tid, user_prio));
			return BCME_USAGE_ERROR;
		}
		user_tid <<= TSPEC_TID_SHIFT;
		user_prio <<= TSPEC_UP_SHIFT;
...
wl_android_cac_ts_config(struct net_device *ndev, char *cmd_argv, int total_len)
{
	...
	/* sync commands from user space */
	mutex_lock(&cfg->usr_sync);
	err = wl_cfg80211_handle_cac_action(cfg, ndev, cmd_argv);
	mutex_unlock(&cfg->usr_sync);
