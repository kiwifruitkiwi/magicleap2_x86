 ...
 *
 * $Id: wl_cfg80211.c 785521 2018-10-19 06:36:45Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
...
static s32 wl_inform_single_bss(struct bcm_cfg80211 *cfg, wl_bss_info_t *bi, bool update_ssid)
{
	...
	/* check the beacon struct version, if struct version is _v2 then copy
	 * bcn timestamp to kernal timestamp in micro seconds
	 */
	if ((bi->length - bi->ie_length) >= (sizeof(wl_bss_info_v109_2_t))) {
		wl_bss_info_v109_2_t *bi_info = (wl_bss_info_v109_2_t *)bi;
		mgmt->u.probe_resp.timestamp = ((u64)bi_info->timestamp[1]*1000000)
					+ ((u64)bi_info->timestamp[0]*1000);
	}
		...
		const wl_event_msg_t *e, void *data)
{
	...
	if (evt_info->version == WL_BSSID_PRUNE_EVT_VER_1) {
		if (evt_info->reason == WLC_E_PRUNE_ASSOC_RETRY_DELAY) {
			...
			SUPP_EVENT(("CTRL-EVENT-BRCM-BSSID-PRUNED", "ssid=%s bssid=" MACF
				" rssi_threshold=%d(dBm) reason=%u timeout_val=%u(ms)\n",
				evt_info->SSID, ETHER_TO_MACF(evt_info->BSSID),
				evt_info->rssi_threshold, reason, evt_info->time_remaining));
		} else {
...
const wl_event_msg_t *event, void *data)
{
	...
	if ((status == WLC_SUP_KEYED || status == WLC_SUP_KEYXCHANGE_WAIT_G1) &&
		...
		peer_addr = (u8 *)wl_read_prof(cfg, ndev, WL_PROF_BSSID);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 15, 0))
		/* NL80211_CMD_PORT_AUTHORIZED supported above >= 4.15 */
		cfg80211_port_authorized(ndev, peer_addr, GFP_KERNEL);
#else
		err = wl_cfgvendor_send_port_authorized_event(wiphy,
			bcmcfg_to_prmry_ndev(cfg),
			(const struct ether_addr *)peer_addr);
...
#endif /* WL_CAC_TS */
int wl_delete_pmk(struct wiphy *wiphy, const u8 *pmk, const u8 *pmk_id,
