 ...
 *
 * $Id: wl_cfgnan.c 784929 2018-10-16 01:01:48Z $
 */
...
wl_cfgnan_suspend_geofence_rng_session(struct net_device *ndev, struct ether_addr *peer)
{
	...
	if (dhd_rtt_get_geofence_rtt_state(dhd) == FALSE) {
		WL_DBG(("Geofencing Ranging not in progress, cancel req dropped\n"));
		goto exit;
	}
...
wl_cfgnan_terminate_directed_rtt_sessions(struct net_device *ndev,
	...
	for (i = 0; i < NAN_MAX_RANGING_INST; i++) {
		ranging_inst = &cfg->nan_ranging_info[i];
		if (ranging_inst->range_id && ranging_inst->range_type == DIRECTED_NAN_RTT) {
...
#ifdef RTT_SUPPORT
	/* suspend ongoing geofence RTT with peer if it is there */
	if ((ret = wl_cfgnan_suspend_geofence_rng_session(ndev, &cmd_data->mac_addr))
			!= BCME_OK) {
		WL_ERR(("Cancel in-progress geofencing failed, ret = %d\n", ret));
		goto fail;
	}
#endif /* RTT_SUPPORT */
		...
		if (event_num == WL_NAN_EVENT_PEER_DATAPATH_IND) {
			*hal_event_id = GOOGLE_NAN_EVENT_DATA_REQUEST;
			...
			if (svc_info) {
				...
				if ((ret = wl_cfgnan_suspend_geofence_rng_session
						(bcmcfg_to_prmry_ndev(cfg), &ev_dp->peer_nmi))
						!= BCME_OK) {
					WL_ERR(("Cancel in-progress geofencing failed,"
						" ret = %d\n", ret));
					goto fail;
				}
#endif /* RTT_SUPPORT */
...
#ifdef RTT_SUPPORT
	int8 index;
	if ((dhd_rtt_get_geofence_target_cnt(dhd) > 0) &&
...
#ifdef RTT_SUPPORT
	if (rng_inst->range_type == DIRECTED_NAN_RTT) {
	...
	bcm_struct_cfgdev *cfgdev, const wl_event_msg_t *event, void *event_data)
{
	...
	if (hal_event_id == GOOGLE_NAN_EVENT_SUBSCRIBE_MATCH) {
		...
		if (nan_event_data->sde_control_flag & NAN_SDE_CF_RANGING_REQUIRED) {
			...
			if (ret == BCME_OK) {
#ifdef RTT_SUPPORT
				if (dhd_rtt_valid_states
					(bcmcfg_to_prmry_ndev(cfg)) == TRUE) {
					/*
					 * If instant RTT not possible,
					 * send discovery result instantly like
					 * incase of NDP connected/connecting
					 * otherwise, disc result will be posted
