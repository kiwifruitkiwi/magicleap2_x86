 ...
 *
 * $Id: dhd_msgbuf.c 785210 2018-10-17 15:54:59Z $
 */
...
#ifdef EWP_EDL
static
int dhd_edl_ring_hdr_write(dhd_pub_t *dhd, msgbuf_ring_t *ring, void *file,
...
dhd_prot_ioctack_process(dhd_pub_t *dhd, void *msg)
{
	...
	dhd->prot->ioctl_ack_time = OSL_SYSUPTIME_US();
...
dhd_prot_ioctcmplt_process(dhd_pub_t *dhd, void *msg)
{
	...
	dhd->prot->ioctl_cmplt_time = OSL_SYSUPTIME_US();
...
int dhd_d2h_h2d_ring_dump(dhd_pub_t *dhd, void *file, unsigned long *file_posn)
{
	...
	if (dhd->dongle_edl_support) {
		ring = prot->d2hring_edl;
		if ((ret = dhd_edl_ring_hdr_write(dhd, ring, file, file_posn)) < 0)
			goto exit;
	}
...
dhd_fillup_ioct_reqst(dhd_pub_t *dhd, uint16 len, uint cmd, void* buf, int ifidx)
{
	...
	prot->ioctl_fillup_time = OSL_SYSUPTIME_US();
