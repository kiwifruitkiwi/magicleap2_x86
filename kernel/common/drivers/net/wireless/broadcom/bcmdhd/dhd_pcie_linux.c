 ...
 *
 * $Id: dhd_pcie_linux.c 784474 2018-10-12 04:48:31Z $
 */
...
dhdpcie_os_chkbpoffset(dhdpcie_info_t *pch, ulong offset)
{
	 ...
	 */
	uint32 bpwin = offset & ~(pch->bar1_size - 1);
...
#ifdef PCIE_AMD_LTR_WAR
	ret = pcie_capability_set_word(bus->rc_dev, PCI_EXP_DEVCTL2,
				PCI_EXP_DEVCTL2_LTR_EN);
	if (ret)
		DHD_ERROR(("%s: Failed to set LTR, %d\n", __FUNCTION__, ret));
#endif
	return ret;
}
