...
#define FTM_DEFAULT_CNT_80M		11
/* To handle congestion env, set max dur/timeout */
#define FTM_MAX_BURST_DUR_TMO_MS	128
...
dhd_rtt_is_nan_peer(dhd_pub_t *dhd, struct ether_addr *peer_mac)
{
	...
	if (ranging_inst) {
		WL_DBG((" RTT peer is of type NAN\n"));
		return TRUE;
	}
	return FALSE;
#else
...
dhd_rtt_nan_start_session(dhd_pub_t *dhd, rtt_target_info_t *rtt_target)
{
	...
	if (ranging_inst->range_status != NAN_RANGING_IN_PROGRESS) {
		...
		if (unlikely(err)) {
			...
			memset_s(&range_res, sizeof(range_res), 0, sizeof(uint8));
			range_res.peer_m_addr = rtt_target->addr;
			wl_cfgnan_reset_geofence_ranging(cfg, ranging_inst);
			goto done;
		}
		ranging_inst->range_type = DIRECTED_NAN_RTT;
	}
done:
	if (err) { /* notify failure RTT event to host */
		WL_ERR(("Failed to issue Nan Ranging Request err %d\n", err));
		wl_cfgvendor_send_as_rtt_legacy_event(cfg->wdev->wiphy,
			bcmcfg_to_prmry_ndev(cfg), &range_res, RTT_STATUS_FAILURE);
	}
	return err;
}
#endif /* WL_NAN */
...
dhd_rtt_get_geofence_target_head(dhd_pub_t *dhd)
{
	...
	if (rtt_status->geofence_cfg.geofence_target_cnt) {
		return &rtt_status->geofence_cfg.geofence_target_info[0];
	} else {
		return NULL;
	}
}
...
dhd_rtt_add_geofence_target(dhd_pub_t *dhd, rtt_geofence_target_info_t *target)
{
	...
	geofence_target_info = rtt_status->geofence_cfg.geofence_target_info;
	memcpy(&geofence_target_info[geofence_target_cnt], target,
			sizeof(rtt_geofence_target_info_t));
...
dhd_rtt_remove_geofence_target(dhd_pub_t *dhd, struct ether_addr *peer_addr)
{
	...
	for (j = index; (j+1) < geofence_target_cnt; j++) {
		if (geofence_target_info[j].valid == TRUE) {
			memcpy(&geofence_target_info[j], &geofence_target_info[j + 1],
					sizeof(rtt_geofence_target_info_t));
		} else {
			break;
		}
...
dhd_rtt_delete_geofence_target_list(dhd_pub_t *dhd)
{
	...
	memset(&rtt_status->geofence_cfg, 0, sizeof(rtt_geofence_cfg_t));
	return err;
}
...
dhd_rtt_sched_geofencing_target(dhd_pub_t *dhd)
{
	...
	if ((dhd_rtt_get_geofence_rtt_state(dhd) == TRUE) ||
			(dhd_rtt_get_role_concurrency_state(dhd) == TRUE)) {
		DHD_RTT(("geofencing rtt already in progress or role concurrency "
			"constraint , sched request dropped\n"));
		return ret;
	}
...
bool
dhd_rtt_valid_states(struct net_device *ndev)
{
	bool ret = FALSE;
	...
	if (wl_get_drv_status(cfg, CONNECTING, ndev)) {
		WL_DBG(("Connection in progress, RTT prohibited\n"));
		goto prohibited;
	}

#ifdef WL_NAN
	if (wl_cfgnan_data_dp_exists(cfg)) {
		WL_DBG(("NDP in progress/connected, RTT prohibited\n"));
		goto prohibited;
	}
#endif /* WL_NAN */

	if (dhd_rtt_get_role_concurrency_state(dhd)) {
		WL_DBG(("ROLE Concurrency, RTT prohibited\n"));
		goto prohibited;
	}

	/* rtt is not prohibited */
	ret = TRUE;
...
void
dhd_rtt_schedule_rtt_work_thread(dhd_pub_t *dhd)
{
	rtt_status_info_t *rtt_status = GET_RTTSTATE(dhd);
	if (rtt_status == NULL) {
		ASSERT(0);
...
dhd_rtt_start(dhd_pub_t *dhd)
{
	...
	/* check for dp/assoc/scan concurrency */
	if (dhd_rtt_valid_states(dev) == FALSE) {
	...
	if ((err = wl_cfgnan_suspend_geofence_rng_session(dev, NULL)) != BCME_OK) {
		DHD_ERROR(("Cancel in-progress geofencing failed, err = %d\n", err));
		return err;
	}
#endif /* WL_NAN */
	...
	if (rtt_target->peer == RTT_PEER_NAN &&
		...
		if (err) {
			DHD_ERROR(("failed to trigger nan-rtt (%d)\n", err));
		} else {
	...
	if (rtt_target->num_frames_per_burst == 0) {
		...
		rtt_target->num_frames_per_burst = MAX(rtt_target->num_frames_per_burst,
			FTM_DEFAULT_CNT_80M);
	}
...
geofence:
	...
	if ((err = dhd_rtt_sched_geofencing_target(dhd)) != BCME_OK) {
		DHD_ERROR(("geofencing sched failed, err = %d\n", err));
	}
#endif /* WL_NAN */
	return err;
	...
	uint16 tlvid, uint16 len)
{
	...
	if (avg_dist == 0xffffffff) {	/* report 'failure' case */
		...
		DHD_RTT((">\tavg_dist=%d.%04dm, burst_num=%d, valid_measure_cnt=%d num_ftm=%d "
			...
			ltoh16_ua(&p_data_info->burst_num),
			p_data_info->num_valid_rtt,
			p_data_info->num_ftm, p_data_info->num_meas)); /* in a session */
	}
	...
	rtt_report->status = ftm_get_statusmap_info(proxd_status,
			&ftm_status_map_info[0], ARRAYSIZE(ftm_status_map_info));
	/* WAR: Change status to no response if no meas frames from peer
	* TODO: Fix in firmware why status is OK even if no meas frames
	*/
	if ((proxd_status == BCME_OK) && !p_data_info->num_meas)
		rtt_report->status = RTT_STATUS_FAIL_NO_RSP;
