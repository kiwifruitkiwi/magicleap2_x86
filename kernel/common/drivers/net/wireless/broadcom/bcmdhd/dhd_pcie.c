 ...
 *
 * $Id: dhd_pcie.c 785540 2018-10-19 09:14:18Z $
 */
...
static void dhd_bigdata_axi_error(dhd_pub_t *dhdp, uint8 *axi_err);
...
static uint pcie_readshared_done = 0;
...
_dhd_bus_pcie_pwr_req_clear_cmn(struct dhd_bus *bus)
{
	...
	if (MULTIBP_ENAB(bus->sih)) {
		...
		if ((bus->sih->buscorerev == 67) && pcie_readshared_done)
			mask = SRPWR_DMN1_ARMBPSD_MASK | SRPWR_DMN3_MACMAIN_MASK |
...
_dhd_bus_pcie_pwr_req_cmn(struct dhd_bus *bus)
{
	...
	if (MULTIBP_ENAB(bus->sih)) {
		...
		if ((bus->sih->buscorerev == 67) && pcie_readshared_done)
		{
			mask = SRPWR_DMN1_ARMBPSD_MASK | SRPWR_DMN3_MACMAIN_MASK |
				SRPWR_DMN2_MACAUX_MASK;
			val = SRPWR_DMN1_ARMBPSD_MASK | SRPWR_DMN3_MACMAIN_MASK |
				SRPWR_DMN2_MACAUX_MASK;
		} else {
			mask = SRPWR_DMN1_ARMBPSD_MASK;
...
{
	dhdp->bus->dpc_sched_time = OSL_SYSUPTIME_US();
}
	...
	do {
		...
		if (intstatus == 0) {
			...
			*/
			...
			bus->last_non_ours_irq_time = OSL_SYSUPTIME_US();
			break;
		}
...
void dhd_dump_intr_counters(dhd_pub_t *dhd, struct bcmstrbuf *strbuf)
{
	...
	bcm_bprintf(strbuf, "resume_intr_enable_count=%lu dpc_intr_enable_count=%lu\n"
		...
		dhd->bus->dpc_return_busdown_count, dhd->bus->non_ours_irq_count);
#ifdef BCMPCIE_OOB_HOST_WAKE
	...
	bcm_bprintf(strbuf, "\ncurrent_time=%llu isr_entry_time=%llu isr_exit_time=%llu\n"
		"dpc_sched_time=%llu, dpc_entry_time=%llu last_process_ctrlbuf_time=%llu "
		...
		dhd->bus->dpc_entry_time, dhd->bus->dpc_sched_time,
		dhd->bus->last_process_ctrlbuf_time, dhd->bus->last_process_flowring_time,
		dhd->bus->last_process_txcpl_time, dhd->bus->last_process_rxcpl_time,
		dhd->bus->last_process_infocpl_time, dhd->bus->dpc_exit_time,
		dhd->bus->resched_dpc_time);
...
dhd_axi_sig_match(dhd_pub_t *dhdp)
{
	...
	if (axi_tcm_addr) {
		...
		if (axi_signature == HND_EXT_TRAP_AXIERROR_SIGNATURE) {
			...
			DHD_ERROR(("%s: Invalid AXI signature: 0x%x\n",
				__FUNCTION__, axi_signature));
			return FALSE;
		}
	} else {
		DHD_ERROR(("%s: Invalid AXI shared tcm address\n", __FUNCTION__));
		return FALSE;
	}
}
...
dhd_axi_error(dhd_pub_t *dhdp)
{
	...
	/* Check if there a FW trap */
	if ((dhdp->bus->api.fw_rev >= PCIE_SHARED_VERSION_6) &&
		(dhdp->bus->dhd->dongle_trap_data = dhd_prot_process_trapbuf(dhdp->bus->dhd))) {
		DHD_ERROR(("%s: Read AXI data from ETD address\n", __FUNCTION__));
		...
		/* Extract the TLVs from the extended trap data */
		tlv = bcm_parse_tlvs(hdr->data, hdr->len, TAG_TRAP_AXI_ERROR);
		if (tlv)
		{
			axi_err = (uint8 *)tlv->data;
			/* Dump data to Dmesg */
			dhd_log_dump_axi_error(axi_err);
			dhd_bigdata_axi_error(dhdp, axi_err);
		}
		else {
			DHD_ERROR(("%s: Error parsing AXI error data\n", __FUNCTION__));
		}
		dhd_bus_handle_mb_data(dhdp->bus, D2H_DEV_FWHALT);
	}
	else if (dhdp->axierror_logbuf_addr) {
		uint32 axi_tcm_addr = dhdpcie_bus_rtcm32(dhdp->bus, dhdp->axierror_logbuf_addr);
		DHD_ERROR(("%s: Read AXI data from TCM address\n", __FUNCTION__));
		size = sizeof(hnd_ext_trap_axi_error_v1_t);
		p_readback_buf = MALLOCZ(dhdp->osh, size);
		if (p_readback_buf == NULL) {
			DHD_ERROR(("%s: out of memory !\n", __FUNCTION__));
			return;
		}
		bcmerror = dhdpcie_bus_membytes(dhdp->bus, FALSE, axi_tcm_addr,
				(uint8 *)p_readback_buf, size);
		if (bcmerror) {
			DHD_ERROR(("%s: error %d on reading %d membytes at 0x%08x\n",
				__FUNCTION__, bcmerror, size, axi_tcm_addr));
			MFREE(dhdp->osh, p_readback_buf, size);
			return;
		}
		axi_err = (uint8 *)p_readback_buf;
		/* Dump data to Dmesg */
		dhd_log_dump_axi_error(axi_err);
		dhd_bigdata_axi_error(dhdp, axi_err);
		MFREE(dhdp->osh, p_readback_buf, size);
	} else {
		DHD_ERROR(("%s: Invalid AXI TCM address\n", __FUNCTION__));
	}
}
...
static void
dhd_bigdata_axi_error(dhd_pub_t *dhdp, uint8 *axi_err)
{
	/* TODO: Send up to Bigdata */
}
#endif /* DNGL_AXI_ERROR_LOGGING */
...
dhdpcie_readshared(dhd_bus_t *bus)
{
	...
	if (bus->sih->buscorerev == 67)
		pcie_readshared_done = 1;
...
dhd_pcie_debug_info_dump(dhd_pub_t *dhd)
{
	...
	DHD_ERROR(("errlog(0x%x)=0x%x errlog_addr(0x%x)=0x%x Function_Intstatus(0x%x)=0x%x "
		"Function_Intmask(0x%x)=0x%x Power_Intstatus(0x%x)=0x%x Power_Intmask(0x%x)=0x%x\n",
