			   ...
			   unsigned long arg)
{
	...
	switch (cmd) {
	case ION_IOC_TEST_ALLOC:
	{
		int fd = -1;

		fd = ion_alloc(data.test_rw.heap_size,
			       data.test_rw.heap_id_mask,
			       data.test_rw.heap_flag);
		data.test_rw.ion_buf_fd = fd;
		break;
	}
