...
#define ICM42605_WHO_AM_I_VAL		0x47
...
#define ICM20602_DFLT_ACC_AVG		ICM20602_ACC_AVG_4_SAMPLES
...
#define ICM20602_DFLT_GYR_AVG		ICM20602_GYR_AVG_4_SAMPLES
...
#ifdef CONFIG_ACPI
static const struct acpi_device_id icm_imu_acpi_match[] = {
	{"INVN2060", ICM_IMU_20602},
	{"INVN4260", ICM_IMU_42605},
	{ },
};
...
static int icm_imu_chip_detect(struct icm20602_drv_data *icm)
{
	int ret;
	struct device *dev = icm->dev;
	const struct acpi_device_id *acpi_id;
	uint8_t whoami;
	uint8_t expected_val = ICM20602_WHO_AM_I_VAL;
	...
	acpi_id = acpi_match_device(ACPI_PTR(icm_imu_acpi_match), dev);
	if (acpi_id && acpi_id->driver_data == ICM_IMU_42605)
		expected_val = ICM42605_WHO_AM_I_VAL;

	if (whoami != expected_val) {
		dev_err(dev, "%s: expected WHO_AM_I 0x%x, got 0x%x!",
			__func__, expected_val, whoami);
		return -EINVAL;
	}
...
static int icm20602_init(struct icm20602_drv_data *icm)
{
	...
	ret = icm_imu_chip_detect(icm);
	if (ret) {
		dev_err(dev, "%s: Failed to detect ICM\n", __func__);
		return ret;
	}
...
static struct spi_driver icm20602_spi_driver = {
	.driver = {
		...
		.acpi_match_table = ACPI_PTR(icm_imu_acpi_match),
	},
