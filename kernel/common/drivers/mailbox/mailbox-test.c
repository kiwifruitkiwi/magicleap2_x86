				       ...
				       size_t count, loff_t *ppos)
{
	...
	print_hex_dump(KERN_INFO, "write message data: ", DUMP_PREFIX_OFFSET,
		       16, 4, tdev->message, 84, true);
...
static void mbox_test_receive_message(struct mbox_client *client, void *message)
{
	...
	buf = kzalloc(MBOX_MAX_MSG_LEN, GFP_KERNEL);
	if (!buf)
		return;

	print_hex_dump(KERN_INFO, "read message data: ", DUMP_PREFIX_OFFSET,
		       16, 4, message, 84, true);
	...
	strncpy(buf, "123456789",
		(MBOX_MAX_MSG_LEN < 10) ? MBOX_MAX_MSG_LEN : 10);

	mbox_set_ack_message(tdev->rx_channel, buf);
				   ...
				   void *message, int r)
{
	...
	mbox_get_ack_message(tdev->tx_channel, buf);

	print_hex_dump(KERN_INFO, "get ack message data: ", DUMP_PREFIX_OFFSET,
		       16, 4, buf, MBOX_MAX_MSG_LEN, true);
	kfree(buf);
	return;
}
...
static int mbox_test_probe(struct platform_device *pdev)
{
	...
	if (res) {
		...
		if (PTR_ERR(tdev->tx_mmio) == -EBUSY)
			...
			tdev->tx_mmio = devm_ioremap(&pdev->dev, res->start,
						     size);
	...
	if (res) {
		...
		if (PTR_ERR(tdev->rx_mmio) == -EBUSY)
			tdev->rx_mmio = devm_ioremap(&pdev->dev, res->start,
						     size);
