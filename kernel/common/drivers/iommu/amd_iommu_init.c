...
void amd_iommu_enable_perfopt(struct pci_dev *pdev)
{
	struct iommu_dev_data *dev_data = NULL;
	struct amd_iommu *iommu = NULL;

	//enable this feature for iommu v2
	...
	if (iommu != NULL) {
		//enable IOMMU Performance Optimization
		writel(1 << 13, iommu->mmio_base +  MMIO_PERF_OPT_OFFSET);
	}
}
EXPORT_SYMBOL(amd_iommu_enable_perfopt);
