...
static irqreturn_t i2s_irq_handler(int irq, void *dev_id)
{
	...
	play_flag = 0;
	cap_flag = 0;
	val = rv_readl(rv_i2s_data->acp3x_base + ACP_EXTERNAL_INTR_STAT);
	...
	if ((val & BIT(I2S_TX_THRESHOLD)) &&
		...
		rv_writel(BIT(I2S_TX_THRESHOLD),
			  rv_i2s_data->acp3x_base + ACP_EXTERNAL_INTR_STAT);
...
static void config_acp3x_dma(struct i2s_stream_instance *rtd, int direction)
{
	...
	for (page_idx = 0; page_idx < rtd->num_pages; page_idx++) {
		...
		rv_writel(low, rtd->acp3x_base + ACP_SCRATCH_REG_0 + val);
		high |= BIT(31);
		rv_writel(high, rtd->acp3x_base + ACP_SCRATCH_REG_0 + val
				+ 4);
	...
	if (direction == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		case I2S_BT_INSTANCE:
			...
			rv_writel(I2S_BT_TX_MEM_WINDOW_START,
				  rtd->acp3x_base + ACP_BT_TX_RINGBUFADDR);
			break;
		...
		default:
			...
			rv_writel(I2S_SP_TX_MEM_WINDOW_START,
				  rtd->acp3x_base + ACP_I2S_TX_RINGBUFADDR);
		}
	} else {
		switch (rtd->i2s_instance) {
		case I2S_BT_INSTANCE:
			...
			rv_writel(I2S_BT_RX_MEM_WINDOW_START,
				  rtd->acp3x_base + ACP_BT_RX_RINGBUFADDR);
			break;
		...
		default:
			...
			rv_writel(I2S_SP_RX_MEM_WINDOW_START,
				  rtd->acp3x_base + ACP_I2S_RX_RINGBUFADDR);
		}
	}
	...
	rv_writel(BIT(I2S_RX_THRESHOLD) | BIT(BT_RX_THRESHOLD)
		...
		rtd->acp3x_base + ACP_EXTERNAL_INTR_CNTL);
}
...
static int acp3x_dma_open(struct snd_pcm_substream *substream)
{
	...
	if (!adata->play_stream && !adata->capture_stream &&
		...
		rv_writel(1, adata->acp3x_base + ACP_EXTERNAL_INTR_ENB);
...
static int acp3x_dma_close(struct snd_pcm_substream *substream)
{
	...
	if (!adata->play_stream && !adata->capture_stream &&
	    !adata->i2ssp_play_stream && !adata->i2ssp_capture_stream)
		rv_writel(0, adata->acp3x_base + ACP_EXTERNAL_INTR_ENB);
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
