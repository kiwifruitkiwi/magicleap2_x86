			     ...
			     int cmd, struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		...
		if (!(rv_readl(rtd->acp3x_base + ACP_BTTDM_ITER) & BIT(0)) &&
		    !(rv_readl(rtd->acp3x_base + ACP_BTTDM_IRER) & BIT(0)))
			rv_writel(0, rtd->acp3x_base + ACP_BTTDM_IER);
		if (!(rv_readl(rtd->acp3x_base + ACP_I2STDM_ITER) & BIT(0)) &&
		    !(rv_readl(rtd->acp3x_base + ACP_I2STDM_IRER) & BIT(0)))
			rv_writel(0, rtd->acp3x_base + ACP_I2STDM_IER);
		ret = 0;
		break;
	default:
		ret = -EINVAL;
