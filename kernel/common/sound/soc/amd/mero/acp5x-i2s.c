			      ...
			      struct snd_soc_dai *dai)
{
	...
	pr_debug("%s i2s_instance:0x%x\n", __func__, rtd->i2s_instance);
	pr_debug("%s format:0x%x rate:0x%x\n", __func__, params_format(params),
		 params_rate(params));
	/* These values are as per Hardware Spec */
	...
	pr_debug("%s master_mode:0x%x\n", __func__, adata->master_mode);
	if (adata->master_mode) {
		...
		pr_debug("%s i2s_instance:0x%x tdm_mode:0x%x\n", __func__,
			 rtd->i2s_instance, adata->tdm_mode);
		/* Todo need to implement separate API to
		 * support more than 2 channels
		 */
		pr_debug("format:0x%x rate:0x%x\n", params_format(params),
			 params_rate(params));
		switch (params_format(params)) {
		case SNDRV_PCM_FORMAT_S16_LE:
		...
		pr_debug("BCLK value:0x%x LRCLK value:0x%x\n", bclk_div_val,
			 lrclk_div_val);
		acp_writel(mclkgen.u32_all, rtd->acp5x_base + master_reg);
		val = acp_readl(rtd->acp5x_base + master_reg);
		pr_debug("I2S Master mode value:0x%x\n", val);
	}
	return 0;
}
			     ...
			     int cmd, struct snd_soc_dai *dai)
{
	...
	rtd = substream->runtime->private_data;
	pr_debug("%s rtd->instance:0x%x\n", __func__, rtd->i2s_instance);
	period_bytes = frames_to_bytes(substream->runtime,
				       substream->runtime->period_size);
	buf_size = frames_to_bytes(substream->runtime,
				   substream->runtime->buffer_size);
	pr_debug("period_bytes:0x%x buf_size:0x%x\n", period_bytes, buf_size);
	switch (cmd) {
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		...
		val = acp_readl(rtd->acp5x_base + water_val);
		pr_debug("WATER MARK LEVEL:0x%x\n", val);

		acp_writel(buf_size, rtd->acp5x_base + buf_reg);
		val = acp_readl(rtd->acp5x_base + buf_reg);
		pr_debug("RING BUFFER SIZE:0x%x\n", val);
	...
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		pr_debug("%s rtd->i2s_instance:0x%x\n", __func__,
			 rtd->i2s_instance);
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			switch (rtd->i2s_instance) {
			case I2S_BT_INSTANCE:
		...
		if (!(acp_readl(rtd->acp5x_base + ACP_BTTDM_ITER) & BIT(0)) &&
		    !(acp_readl(rtd->acp5x_base + ACP_BTTDM_IRER) & BIT(0)))
			acp_writel(0, rtd->acp5x_base + ACP_BTTDM_IER);
