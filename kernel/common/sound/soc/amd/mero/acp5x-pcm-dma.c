...
static irqreturn_t i2s_irq_handler(int irq, void *dev_id)
{
	...
	play_flag = 0;
	cap_flag = 0;
	val = acp_readl(rv_i2s_data->acp5x_base + ACP_EXTERNAL_INTR_STAT);
	if ((val & BIT(BT_TX_THRESHOLD)) && rv_i2s_data->play_stream) {
		acp_writel(BIT(BT_TX_THRESHOLD), rv_i2s_data->acp5x_base +
	...
	if ((val & BIT(I2S_TX_THRESHOLD)) &&
		...
		acp_writel(BIT(I2S_TX_THRESHOLD),
			   rv_i2s_data->acp5x_base + ACP_EXTERNAL_INTR_STAT);
		snd_pcm_period_elapsed(rv_i2s_data->i2ssp_play_stream);
		play_flag = 1;
	}

	if ((val & BIT(BT_RX_THRESHOLD)) && rv_i2s_data->capture_stream) {
		acp_writel(BIT(BT_RX_THRESHOLD), rv_i2s_data->acp5x_base +
			   ACP_EXTERNAL_INTR_STAT);
...
static void config_acp5x_dma(struct i2s_stream_instance *rtd, int direction)
{
	...
	if (direction == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		case I2S_BT_INSTANCE:
			...
			acp_writel(I2S_BT_TX_MEM_WINDOW_START,
				   rtd->acp5x_base + ACP_BT_TX_RINGBUFADDR);
			break;
		...
		case I2S_BT_INSTANCE:
			...
			acp_writel(I2S_BT_RX_MEM_WINDOW_START,
				   rtd->acp5x_base + ACP_BT_RX_RINGBUFADDR);
			break;
	...
	acp_writel(BIT(I2S_RX_THRESHOLD) | BIT(BT_RX_THRESHOLD)
		| BIT(I2S_TX_THRESHOLD) | BIT(BT_TX_THRESHOLD),
		rtd->acp5x_base + ACP_EXTERNAL_INTR_CNTL);
	value = acp_readl(rtd->acp5x_base + reg_dma_size);
	pr_debug("DMA_SIZE: 0x%x\n", value);
	value = acp_readl(rtd->acp5x_base + reg_fifo_addr);
	pr_debug("FIFO_ADDR: 0x%x\n", value);
	value = acp_readl(rtd->acp5x_base + reg_fifo_size);
	pr_debug("FIFO_SIZE: 0x%x\n", value);
}

static int acp5x_dma_open(struct snd_pcm_substream *substream)
{
...
static int acp5x_dma_hw_params(struct snd_pcm_substream *substream,
...
static snd_pcm_uframes_t acp5x_dma_pointer(struct snd_pcm_substream *substream)
{
	struct i2s_stream_instance *rtd;
	u32 pos;
	u32 buffersize;
...
static int acp5x_dma_new(struct snd_soc_pcm_runtime *rtd)
{
...
static int acp5x_dma_hw_free(struct snd_pcm_substream *substream)
{
	return snd_pcm_lib_free_pages(substream);
}

static int acp5x_dma_mmap(struct snd_pcm_substream *substream,
			  struct vm_area_struct *vma)
{
	return snd_pcm_lib_default_mmap(substream, vma);
}

static int acp5x_dma_close(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *prtd;
	struct snd_soc_component *component;
	struct i2s_dev_data *adata;
	...
	if (!ins)
		return -EINVAL;
	pr_debug("%s i2s_instance:0x%x substream:0x%x\n", __func__,
		 ins->i2s_instance, substream->stream);
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
...
static struct snd_pcm_ops acp5x_dma_ops = {
	.open = acp5x_dma_open,
	.close = acp5x_dma_close,
	.ioctl = snd_pcm_lib_ioctl,
	.hw_params = acp5x_dma_hw_params,
	.hw_free = acp5x_dma_hw_free,
