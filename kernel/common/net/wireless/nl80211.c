			  ...
			  enum nl80211_iftype iftype)
{
	...
	if (!cfg80211_chandef_usable(&rdev->wiphy, chandef,
				     IEEE80211_CHAN_DISABLED, iftype)) {
				 ...
				 struct genl_info *info)
{
	...
	result = nl80211_parse_chandef(rdev, info, &chandef, iftype);
	if (result)
		return result;
					 ...
					 struct genl_info *info)
{
	...
	err = nl80211_parse_chandef(rdev, info, &chandef, wdev->iftype);
	if (err)
		return err;
...
skip_beacons:
	err = nl80211_parse_chandef(rdev, info, &params.chandef, wdev->iftype);
	if (err)
		return err;
