...
int sig_verify_elf_signature(struct file *file)
{
	...
	rd_off = 0;
	if (file_len != kernel_read(file, file_buf, file_len, &rd_off)) {
		err = -EIO;
	...
	inf.len = file_len;
	inf.name = "elf_sign";
	err = mod_verify_sig(file_buf, &inf);
	if (err) {
...
bool sig_check_if_ml_internal(struct file *file)
{
	...
	key_id = pkcs7->signed_infos->sig->auth_ids[0];
	if (key_id == NULL)
		goto out;

	if (key_id->len == sizeof(ML_INTERNAL_ID)) {
		retval = (crypto_memneq(ML_INTERNAL_ID, key_id->data, key_id->len) == 0);
	} /* TODO: Consolidate TMP ID with original */
	else if (key_id->len == sizeof(ML_INTERNAL_ID_TMP)) {
		retval = (crypto_memneq(ML_INTERNAL_ID_TMP, key_id->data, key_id->len) == 0);
	}
			       ...
			       size_t plen)
{
	...
	struct key *builtin_keyring = get_builtin_trusted_keys();

	if ((payload == NULL) || (plen == 0))
		return 0;

	if ((key_ref_to_ptr(keyring_ref) != builtin_keyring) &&
		...
		system_keyring_equiv_key = find_asymmetric_key(
						       builtin_keyring,
						       x509_cert->id, NULL,
						       true);
