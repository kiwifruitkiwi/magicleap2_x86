...
loff_t mod_seek_module_signature(struct file *file)
{
	...
	char buf[sizeof(struct module_signature) + sizeof(MODULE_SIG_STRING) - 1];
	struct module_signature *ms;
	loff_t pos, filelen, tmppos;
	ssize_t read_len;
	size_t sig_len;
	...
	tmppos = pos; /* needed since kernel_read modifies this and we need it after */
	read_len = kernel_read(file, buf, sizeof(buf), &tmppos);
...
u8 *mod_get_module_signature(struct file *file, size_t *out_overall_len, size_t *out_sig_len)
{
	...
	read_len = kernel_read(file, retval, overall_len, &pos);
