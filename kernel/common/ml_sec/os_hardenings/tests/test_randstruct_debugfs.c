...
static struct dentry *g_mlsec_dir = 0;
static struct dentry *g_randstruct_file = 0;
...
static void randstruct_test_exit(void)
{
	...
	debugfs_remove(g_mlsec_dir);
}

static int randstruct_test_init(void)
{
	g_mlsec_dir = debugfs_create_dir("ml_sec", NULL);
	if (g_mlsec_dir == NULL)
		return -ENOMEM;

	g_randstruct_file = debugfs_create_file("randstruct_test", 0400, g_mlsec_dir, NULL, &randstruct_test_fops);
